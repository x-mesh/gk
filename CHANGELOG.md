...
- **`gk session audit`와 PreToolUse 힌트가 대체할 수 없는 명령을 가리키던 분류 네 가지를 고쳤다.** 분류기(`gitSegmentFinding`)는 감사 집계·실시간 힌트·턴 절감 계산이 함께 쓰므로, 틀린 분류는 지표를 부풀리는 동시에 에이전트에게 다른 일을 하는 명령을 안내했다. `git rebase --continue`·`merge --abort`·`cherry-pick --continue`는 새 부류 `raw-op-control`(`git-kit continue`/`abort`)로 옮겼다 — 종전엔 `raw-integration`으로 묶여 해결 중인 통합 위에 `pull`/`sync`/`merge`를 또 권했다. `git cherry-pick <sha>`와 `rebase --skip`은 대체 동사가 없으므로 gap(`uncovered-raw-git`)으로 보낸다. `git branch --contains`는 `git-kit find`로 안내하던 것을 멈췄다: find는 커밋을 찾을 뿐 "어느 브랜치가 이 커밋을 담고 있나"에는 답하지 못한다(읽기 전용이라 gap으로도 세지 않는다). `raw-release-sequence`는 같은 세션에 **태그 생성**과 push가 함께 있을 때만 잡는다 — `git tag --list` 조회도 신호로 세던 탓에 태그 목록 확인과 기능 브랜치 push가 릴리스로 계상됐다. 릴리스가 아닌 push는 어디에도 잡히지 않았는데, 이제 감사에서 `raw-push`(`git-kit push`)로 센다 — PreToolUse 힌트는 종전대로 단독 push를 인라인으로 안내하지 않는다. `git fetch`도 gap으로 옮겼다 — 힌트가 `git-kit pull`을 권해, fetch만 하려던 에이전트가 따르면 upstream이 현재 브랜치에 통합됐다. `git-kit pull --fetch-only`는 현재 브랜치의 upstream 하나만 가져오므로 `git fetch --all`·`git fetch origin`의 대체가 아니다. 같은 이유로 "fetch 후 merge"를 integration 턴 묶음에서 뺐다(`gk merge`와 `gk sync`는 fetch하지 않는다). `batch --plan` 합성도 fetch·op-control·cherry-pick을 다른 명령으로 바꾸지 않고 omitted로 남긴다. 최근 30일 감사 기준으로 `raw-integration`은 112건에서 22건으로, uncovered는 12건에서 98건(fetch 79건 포함)으로 바뀌었으므로, 이 버전 전후의 채택률·gap 추세는 직접 비교할 수 없다.
...
- **저장소 레이아웃 캐시가 삭제된 뒤 재생성된 워크트리·저장소에 낡은 경로를 계속 서빙하던 문제.** 종전엔 "common dir이 존재하면 유효"로만 판정해, 같은 경로에 다른 저장소가 들어서거나(형제 워크트리가 옛 common dir을 붙들고 있는 경우) 워크트리 제거 후 admin 디렉터리가 다른 워크트리에 재할당되는 경우(`git worktree add`의 이름 재사용)를 놓쳤다 — 후자는 `gitstate.Detect`가 리베이스 중인 워크트리를 엉뚱한 워크트리의 진행 상태로 잘못 보고할 수 있어, 히스토리를 바꾸는 명령의 게이트가 틀린 답을 줄 위험이 있었다. 새 공용 검사가 워크트리→레이아웃 링크(공용 디렉터리와 개별 gitdir 양쪽)를 직접 확인한다.
- **트렁크·헤더 캐시가 일부 환경에서 답을 영영 못 바꾸던 문제 여러 건을 닫았다.** 트렁크 조회가 타임아웃 등으로 실패하면 빈 답이 영구 캐시돼 fork 컬럼과 land-ready 판정이 사라졌고, reftable ref 백엔드에서는 지문이 보는 파일들이 애초에 존재하지 않아 트렁크와 `gk status --watch` 헤더가 첫 값에 얼어붙었으며, `GIT_DIR`/`GIT_COMMON_DIR`이 export된 환경에서는 반대로 레이아웃 캐시가 영구 미스였다. `--repo`를 지정하지 않는 기본 호출(`gk switch`, `gk worktree`, 단일 저장소 `gk watch`)에서는 캐시 스코프가 빈 디렉터리를 "알 수 없음"으로 오인해 관련 캐시가 통째로 꺼져 있기도 했다. 전부 지문·스코프에 빠졌던 입력을 채우거나 실패를 캐시하지 않도록 고쳤다.
...
- **`gk watch`의 폴 1회 원가를 git 서브프로세스 267개에서 110개로 줄였다 (repo 25개·워크트리 59개 기준, CPU 500% → 16%).** 빈 `git` fork 하나가 약 10ms CPU라 폴 비용은 호출 **횟수**에 선형인데, 폴마다 다시 묻던 것들 중 상당수는 답이 이미 정해져 있었다. 저장소가 이미 쓰던 방식 — 비교 대상의 커밋 tip을 키에 넣으면 tip이 움직이는 순간 다른 키가 되어 stale이 원리적으로 불가능한 콘텐츠 키 메모이제이션(`forkPointCache`) — 을 남은 자리에 마저 적용했다. 워크트리마다 돌던 `rev-list --count <branch>..<parent>`와 `merge-base --is-ancestor`는 둘 다 두 커밋의 순수 함수이고 tip은 브랜치 목록에서 이미 딸려 온다(폴당 20 + 40 → 0). repo마다 돌던 트렁크 조회(`symbolic-ref`)와 `branch.<name>.gk-parent` 배치 읽기(`config --get-regexp`)는 각각 origin/HEAD·packed-refs·refs/heads와 config 파일의 mtime을 키로 삼는다 — fork 하나(약 10ms)를 `stat` 두어 번(수 µs)으로 바꾸는 교환이다(폴당 25 + 24 → 0). 피드의 `+/-` 수치와 함수 이름은 변경 파일의 지문(porcelain XY + mtime + 크기)의 함수인데 그 지문은 스캔이 이미 모으고 있었으므로, 지문이 그대로면 `git diff -U0` 두 번과 untracked 파일 읽기를 건너뛴다(폴당 39 → 6; `git add`만 해도 XY가 바뀌므로 staging 전환은 그대로 잡힌다). 크기를 함께 보는 이유는 타임스탬프 해상도가 편집 간격보다 거친 파일 시스템 때문이다 — 한 tick 안에 들어온 두 번의 저장은 mtime으로 구분되지 않는다. 같은 tick에 길이까지 같은 편집은 여전히 구분되지 않는데, 그걸 막으려면 변경 파일을 매번 읽어야 하고 그게 이 경로가 피하려는 비용이다. 단일 워크트리 피드에서는 **clean 트리인데도** `git diff`를 두 번 돌리던 자리(fleet 쪽 쌍둥이에는 있던 가드가 여기엔 없었다)를 막고, 헤더 5개 호출(브랜치·업스트림·ahead/behind·HEAD 제목)은 HEAD·packed-refs·해당 ref 파일이 하나도 안 움직였으면 통째로 재사용한다 — 편집 중에는 ref가 움직이지 않으므로 정상 상태에서 0이 된다. `worktree list`는 캐시하지 않는다: 에이전트가 `git switch` 한 번으로 바꾸는 값이고, 감독용 대시보드가 낡은 브랜치를 보여주는 대가는 fork 25개보다 크다. 렌더 값과 `--json`/`--events` 계약은 위 지문 한계를 제외하면 그대로다.
...
- **Homebrew Cask 설치 뒤 macOS가 unsigned `gk`를 격리해 실행 파일을 휴지통으로 옮기던 문제를 피한다.** `.goreleaser.yaml`은 생성한 Cask의 declarative `postflight_steps`에서 staged `gk` 파일의 `com.apple.quarantine` 확장 속성을 제거한다. GoReleaser가 Cask 템플릿을 다시 렌더하므로 Homebrew의 `{{staged_path}}` 토큰은 이중 이스케이프한다.
...
- **agents:** default contract install to global scope

### Fixed

- **cli:** clarify occupied worktree recovery
...
- **`gk session audit --record`가 프로젝트별 내역(`by_project`)도 남긴다 — 채택률이 흔들렸을 때 "왜"를 사후에 물을 수 있도록.** 기록된 한 줄은 그 시점의 집계만 담고 있었다: `adoption_rate`가 36.7% → 15.5%로 떨어져도, 사람들의 습관이 나빠진 것인지 **스캔 창의 구성이 바뀐 것**인지 구분할 방법이 없었다. `--record`는 최신 N개 세션 파일만 훑으므로 조용해진 프로젝트는 창에서 빠지고 새 프로젝트가 들어온다 — raw git을 많이 쓰는 미온보딩 프로젝트 하나가 창의 40%를 차지하면 아무의 습관도 변하지 않은 채로 집계가 몇 포인트씩 움직인다. 실제로 그런 급락이 한 번 있었고, 판정은 "원인 불명"으로 끝날 수밖에 없었다. 이제 항목마다 프로젝트별 `files`/`raw_git`/`git_kit`/`gk_short`가 함께 남아 두 항목을 비교하면 구성 변화가 바로 드러난다(실측 200파일 창 15개 프로젝트 = 약 1.5KB/줄, 주 1회 append).
  ...
  전체 코퍼스에 `--since` 창을 겹쳐 보는 것으로는 이 질문에 답할 수 없다 — 그건 다른 표집 축이라, 한쪽으로 다른 쪽의 값을 설명하면 반증 불가능한 이야기가 된다. `HistoryEntry`가 재사용 대신 전용 최소 타입(`ProjectShare`)을 쓰는 것도 같은 이유다: 이 파일은 시간을 가로질러 읽히므로, 리포트의 프로젝트 행에 필드가 하나 붙을 때마다 형식이 따라 흔들리면 안 된다. `rate`는 저장하지 않는다 — 합에서 유도되는 값을 굳이 저장하면 나중에 자기 자신과 어긋날 수 있는 것이 하나 더 생긴다. 이전 항목에는 이 필드가 **없다**(0이 아니라 그때는 재지 않았다는 정직한 신호).
...
- **`gk session audit`이 gk 자신의 재조회도 센다 — 채택이 오를수록 지표가 눈멀던 자리.** 기존 턴 지표는 딱 하나를 묻는다: "gk를 **집었다면** 몇 왕복이 줄었나". 그래서 `commandPayloadTrivial`이 git-kit을 부른 턴을 통째로 할인한다 — 설계상 옳지만, `gk context`를 세 턴에 걸쳐 세 번 부른 낭비는 **정의상 0으로 보인다.** 그리고 그 사각지대는 채택률이 오를수록 커진다(13.1% → 36.7%): 남은 낭비가 정확히 지표가 못 보는 쪽으로 이동한다. 이제 같은 탐지기를 렌즈로 나눠 git-kit 세그먼트에도 돌린다 — 간격 허용치·변경 장벽·페이징 가드·주 그룹 귀속까지 전부 공유하고, 어느 바이너리의 세그먼트를 읽는지만 다르다. 결과는 `turns.gk_reprobe`로 **따로** 나간다: 위의 숫자에 절대 더하지 않는다. 합치면 아무 의미도 없는 수가 나올 뿐 아니라, `~/.gk/audit-history.jsonl`이 조용히 세 번째 기준선을 얻는다 — 그 파일은 이미 읽는 사람에게 "경계를 넘어 비교하지 말라"고 경고하는 불연속을 둘이나 갖고 있다. `--record`도 `gk_turns`/`gk_reprobe_saved`를 **덧붙이기만** 한다(이전 항목엔 없음 = 그때는 재지 않았다는 정직한 신호).

  **그리고 이 기능은 자기 자신을 반증했다.** 실측: 최근 7일 **0 / 430 gk 턴**, 30일 **1 / 1465**. 처음엔 1건이 잡혔는데 들여다보니 `gk diff --help` 다음의 `gk diff --digest main...HEAD`였다 — `--help`는 저장소를 읽지도 않으므로 재조회가 아니라 "동사를 익히고 나서 쓴 것"이라, 오탐으로 판정해 제외했다. 즉 이 낭비는 **지금은 없다.** 그래도 계측을 남기는 이유는, 없다는 것을 이제 근거를 갖고 말할 수 있고 채택이 더 오를 때 생기면 잡히기 때문이다. 미지의 verb는 read-only가 아니라 **변경으로 간주**한다 — 잘못 변경으로 보면 절감 하나를 포기할 뿐이지만, 잘못 read-only로 보면 일어나지도 않은 상태 변화를 가로지르는 절감을 주장하게 된다.

- **`git checkout -- <경로>`처럼 되돌릴 수 없는 명령에 힌트가 초록불 대신 경고를 준다.** gk가 못 덮는 명령에는 **침묵**이 정답이다 — 답할 수 없는 질문으로 에이전트를 떠미는 것이 이 분류기 분할이 없애려던 과대주장이니까. 그런데 이 한 형태만은 침묵이 **승인처럼 읽힌다**: `gk hint 'git checkout -- src/a.rs'`가 `ok: no git-kit replacement`라고 답했고, 이건 gk 미커버 명령 중 **커밋 안 된 작업을 되돌릴 수 없이 날리는 유일하게 흔한 명령**이다(실측 30일 18건). 그래서 대체재를 주장하지 않는 별도 축을 뒀다 — `HintResult.Caution`은 `Covered`를 false로 둔 채 "대체 동사는 없다, 다만 `gk snapshot`을 먼저 돌리면 `refs/wip`에 안전망이 남아 되돌릴 수 있다"고만 말한다. 경고는 **절대 차단하지 않는다**: 강제로 보낼 대체재가 없고, 정당한 discard를 막으면 훅을 우회하는 재시도를 부를 뿐이다. 같은 이유로 `hintSeen` 중복 제거도 건너뛴다 — 이건 이미 가르친 교훈이 아니라 **지금 이 명령의 결과**에 대한 경고다. `git checkout <브랜치>`(HEAD 이동)와 `git restore --staged`(인덱스만, `gk unstage`가 덮음)는 제외된다.

### Fixed

- **`gk session audit --full`이 약속만 하고 안 주던 "uncapped evidence"를 실제로 준다.** `--full`의 도움말은 처음부터 "uncapped run commands and evidence"였는데, 상한(`maxEvidence = 5`)이 **수집 시점**에 걸려 있었다 — `--full`은 이미 5건에서 멈춘 데이터의 직렬화만 uncap하고 있었던 것이다. count 425짜리 finding을 5개 샘플로 판단해야 했고, 분포가 필요하면 코퍼스를 다시 grep하는 수밖에 없었다(이번 roadmap 세션에서 실제로 두 번 그랬다). 이제 `--full`이 수집 상한도 함께 올린다: 실측으로 covered finding들이 count와 같은 수의 evidence를 낸다(355/355). `uncovered-raw-git`의 subcommand당 1건 샘플링은 그대로다 — 그건 상한이 아니라 "빈도 높은 subcommand가 슬롯을 독식해 희귀한 것이 예시 없이 출하되는" 문제를 막는 의도된 설계라서다.

- **`gk log --json`이 첫 커밋을 뺀 모든 레코드의 필드를 한 칸씩 밀어서 내보내던 문제.** `sha`는 빈 문자열, `short_sha`에는 40자 전체 SHA, `author`에는 짧은 SHA, `date`에는 이메일, `subject`에는 ISO 날짜가 들어가고 진짜 제목은 `body`로 밀려나 있었다 — `-n 1`만 우연히 멀쩡했다. 원인은 이 호출이 `-z`를 함께 넘긴다는 것이다: 그러면 git이 우리가 넣은 `%x1e` **뒤에** 자기 몫의 NUL을 하나 더 붙이므로, `\x1e`로 자른 두 번째 이후 레코드는 전부 NUL로 시작하고 필드 분리에서 맨 앞에 빈 칸이 하나 생긴다. 이제 레코드마다 선행 구분자를 벗겨낸 뒤 자른다. 잘못된 JSON은 조용해서 더 나쁘다 — 파싱은 성공하고 값만 틀리므로, 이 출력을 믿는 에이전트는 틀렸다는 사실을 알아차리는 데만 왕복을 쓴다. 회귀 테스트의 픽스처가 `-z`가 실제로 쓰는 종료자를 빠뜨리고 있었던 것이 이 버그가 초록불로 출하된 이유라, 픽스처를 진짜 출력 모양으로 바꾸고 두 번째 레코드의 **모든** 필드를 검사하게 했다(제목과 본문만 보면 한 칸 밀림이 그대로 통과한다). 마지막 레코드의 빈 본문 구분자까지 함께 깎으면 필드가 여섯 개로 줄어 그 커밋이 통째로 사라지므로, 그 경우도 픽스처에 넣어 고정했다. 여기서 한 걸음 더 나갔다: 손으로 만든 픽스처는 이 계약을 끝까지 지킬 수 없다 — 픽스처가 틀리면 테스트가 틀린 출력을 정답으로 배운다(이 버그가 정확히 그렇게 출하됐다). 그래서 **골든 왕복 테스트**를 추가했다. 진짜 repo에서 `gk log --json`을 돌리고 **모든 레코드의 모든 필드**를 git 자신에게 물어본 값(`git show -s --format=%H/%h/%an/…`)과 대조한다 — 그 바이트 스트림의 유일한 권위는 git이니까. 실제로 옛 파서를 되돌려 보면 이 테스트가 즉시 잡는다. count와 첫 레코드만 보던 기존 통합 테스트가 놓친 것도 같은 이유로 고정된다: 밀림은 레코드 0은 멀쩡하고 개수도 맞았다.
...
- **`gk watch`의 변경 피드를 스크롤할 수 있다 — 화면 위로 밀려난 이벤트가 이제 읽을 수 있는 것이 된다.** 피드는 시작부터 화면보다 많은 이벤트를 들고 있었지만 보여주는 것은 언제나 최신 꼬리뿐이었다. 에이전트가 빠르게 편집하는 동안 한 번 눈을 뗀 사이에 한 화면치가 지나가면 **그 사이에 무엇이 바뀌었는지 확인할 방법이 아예 없었다** — 메모리에는 남아 있는데 도달할 수단이 없는 상태였다. pgup/pgdn이 한 페이지씩, shift+↑/↓(또는 J/K)가 한 줄씩 거슬러 올라가고, home은 아직 남아 있는 가장 오래된 이벤트로, end는 라이브 꼬리로 돌아온다. 줌 뷰와 `gk status --watch`에서는 커서가 없으므로 방향키만으로 같은 일을 한다. 스크롤 위치를 **끝에서부터의 거리**로 재는 것이 핵심이다: 그래야 새 이벤트가 도착해도 읽고 있던 줄이 그대로 있는다(앞에서부터 세면 이벤트가 하나 들어올 때마다 화면이 밀려 읽기가 불가능하다). 링 버퍼가 가득 찬 뒤에는 `len(feed)`이 늘지 않으면서 꼬리만 움직이므로, 얼마나 늘었는지는 추가 개수를 직접 세어 알려 준다. 스크롤 중에는 구분선이 **아래에 몇 개가 기다리는지와 돌아가는 키**를 함께 표시한다 — 꼬리를 따라가지 않는 피드와 아무 일도 일어나지 않는 피드는 화면상 구별되지 않고, 라이브 대시보드를 보는 사람이 그것을 추측하게 두면 안 된다. 대시보드 피드의 보관량은 200개에서 1,000개로 올렸다: 화면 밖 이력에 이제 독자가 생겼기 때문이다(단독 피드는 이미 1,000개였다).
...
- **`gk chat`이 gk 명령을 말하기 전에 실제 명령 트리에서 찾아본다 — `gk_suggest`.** chat의 다른 도구는 전부 저장소를 읽으므로 모델의 주장은 구조적으로 근거를 갖는다. 그런데 "다음에 뭘 실행하면 되나"라는 제안만은 본질적으로 앞을 보는 말이라 저장소의 무엇도 그것을 확인해 줄 수 없고, 그대로 두면 모델은 **이 빌드에 없는 하위명령과 플래그를 확신에 차서 지어낸다**. `gk_suggest`는 그 한 걸음을 발명이 아니라 조회로 바꾼다 — 살아 있는 cobra 트리를 걸어 의도에 맞는 명령의 전체 경로·한 줄 요약·주요 플래그·사용 예를 돌려주므로, 명령이 추가·개명·삭제되면 아무도 표를 고치지 않아도 제안 가능한 목록이 따라 바뀐다. 관련성 게이트가 따로 있다: 약 90개 명령의 도움말은 산문이라 어떤 문장이든 **어느 하나와는 단어가 겹친다**("deploy to kubernetes and order pizza"가 `gk batch`의 "an ordered JSON plan" 속 "order"로 걸린다). 점수만으로는 이 우연을 진짜 일치와 가를 수 없으므로, 명령 자신의 이름·별칭에 걸리거나 서로 다른 두 단어가 동시에 맞을 때만 통과시킨다. 못 찾으면 빈 결과가 나가고, 시스템 프롬프트는 그것을 "gk에 그런 명령이 없다"로 읽으라고 못박는다 — **조용한 도구가 자신만만한 오답보다 낫다**. 함께 들어간 프롬프트 규칙은 세 가지다: 답변에 gk 명령을 적기 전에 반드시 조회할 것, 코드를 이해하려고 이 도구를 잡지 말 것(그건 `git_grep`/`file_read`/`git_log`의 일이다), 후속 제안은 정말 쓸모 있을 때 **최대 하나만** 달 것 — 매 턴 붙는 제안은 광고처럼 읽혀 독자가 건너뛰는 법을 배운다.

### Changed

- **도구 호출 줄이 인자와 결과를 사람이 쓸 표현으로 요약하고, 열을 맞춘다.** 이 줄은 흘러가는 대화록을 세로로 훑어 읽는 표다. 그래서 화살표가 매번 같은 칸에 떨어지는 것이 핵심이다 — "git_log를 세 번 불렀는데 전부 빈 결과"가 읽어서 알아내야 하는 것이 아니라 한눈에 보이게 만드는 게 그 정렬이다. `▸ 이름 · 인자 → 결과 · 소요시간` 배치를 80칸(설계할 값어치가 있는 가장 좁은 폭)에 정확히 맞추되, 결과 칸만은 터미널에 남는 폭을 전부 가져간다. 22칸에 고정하면 실제 요약이 단어 중간에서 잘려(`4 changed · 8 untrack…`) 그 칸을 만든 이유인 정보를 정확히 잃기 때문이다. 인자는 그것을 만든 JSON이 아니라 `develop · 5` 같은 구절로 적고, 모르는 도구나 파싱 실패는 압축 JSON으로 물러난다 — 나중에 추가된 도구가 아무것도 안 보여주는 대신 오늘의 동작으로 강등된다. 실패한 줄은 결과 칸을 그대로 재사용하므로 성공한 줄과 폭이 같다.

### Fixed

- **OpenAI 계열 모델이 인자를 자기 내부 봉투에 싸서 보낼 때 첫 라운드를 통째로 버리던 문제.** 이 모델들은 이따금 `{"recipient_name":"functions.git_log","parameters":{...}}`처럼 멀티툴 디스패처의 포장을 그대로 실어 보낸다. **인자는 맞고 포장만 틀린** 상황인데, 이걸 거부하면 모델이 실제로 하지 않은 실수에 왕복 한 번을 쓰고, 그것도 아직 아무 증거도 모으지 않은 **첫 라운드**를 태운다. 이제 `Dispatch`가 봉투를 벗겨 넘긴다 — 어떤 도구가 불렸는지 아는 자리가 거기뿐이라 그 자리여야 하고, 덕분에 모든 핸들러가 따로 손대지 않아도 혜택을 받는다(redaction·capping이 거기 사는 것과 같은 이유다). 봉투가 **명백하지 않은 것은 손대지 않는다** — 오탐으로 진짜 인자를 조용히 버리는 쪽이 진짜 봉투에서 나는 에러보다 훨씬 나쁘다. 다만 `recipient_name`이 실제로 불린 것과 **다른** 도구를 가리키면 벗기지 않고 거부한다: 그건 모델이 A를 지목하고 B를 부른 것이라 인자는 A의 것이고, 그대로 B에 먹이면 헷갈리는 에러를 내거나 더 나쁘게는 엉뚱한 인자로 성공해 버린다. 에러가 양쪽 이름을 다 밝히므로 모델이 한 번에 고칠 수 있다.
- **`gk watch`의 디스크립터가 저장소에 새 파일이 생길 때마다 1:1로 늘어나던 문제 — 앞선 네 번의 fd 수정이 모두 비껴간 경로다.** kqueue에서 감시 중인 디렉터리에 파일이 생기면, 그 파일을 감시하기로 결정하는 것은 gk가 아니라 fsnotify다. 디렉터리에 `NOTE_WRITE`가 뜨면 fsnotify는 `dirChange` → `sendCreateIfNew` → `internalWatch`를 거쳐 **새 파일마다 디스크립터를 하나씩 연다**. 이 일이 fsnotify 안에서 벌어지므로 gk의 회계는 볼 방법이 없었다 — 시작 시점의 트리 걷기는 이미 끝났고, 런타임 승인 경로(`admitRuntimeDir`)는 새 **디렉터리**만 본다. 그래서 `fw.cost`는 시작값에 얼어붙은 채 실제 디스크립터 수만 올라갔다. `WatchList()`로 확인할 수도 없다 — 그건 명시적으로 `Add`한 경로만 돌려주므로(파일 500개를 감시하는 watcher가 3을 보고한다), **관측 가능한 신호가 애초에 없었다**는 것이 이 누수가 네 번 살아남은 이유다. 이제 이벤트 루프가 파일 생성을 예산에 과금하고, 삭제·이름변경은 되돌려준다(kqueue의 `remove()`가 그 fd를 닫으므로, 임시 파일의 생성·삭제 반복이 예산을 한 방향으로 밀어 올리면 안 된다). 예산을 넘기면 EMFILE 때와 같은 경로로 watcher를 통째로 놓아 주고 그 트리를 폴링으로 내린다. 이건 예방이 아니라 탐지다 — fsnotify가 이벤트를 넘기기 전에 이미 fd를 열기 때문에 폭주 구간에서 초과가 먼저 일어난다. 보장하는 것은 **초과 폭이 유계이고 그 뒤 전량 반납된다**는 것이고, 그게 정상 상태를 유한하게 만든다.
- **그리고 예산 자체가 사실상 무한이었다 — 상한을 65,536에서 8,192로 내린다.** 예산은 fd soft limit의 1/3로 잡았는데, 그 soft limit을 올리는 것이 gk 자신이다(`raiseFDLimit`). 셸이 1<<20을 물려주면 나눗셈은 의미를 잃고 `1<<16` 상한이 실제 숫자가 된다 — watcher **하나**가 디스크립터 6만 5천 개를 쥐어도 정당한 셈이다. 실제 트리는 거기 근처에도 못 간다(이 저장소가 약 900개다). watcher의 가치는 트리 크기에 비례해 커지지 않는 반면 비용은 프로세스 전체가 나눠 진다 — 여기서 쥔 디스크립터가 곧 이후 모든 git 서브프로세스에 필요한 그 디스크립터다. 그래서 상한은 "프로세스가 감당 가능한 최대"가 아니라 "watcher가 값어치할 수 있는 최대"를 뜻하게 바꾼다. 그보다 큰 트리는 폴링이 더 나은 거래다.
- **스스로 물러난 watcher가 재시도 쿨다운을 걸지 않아, 다음 동기화가 곧바로 같은 벽에 다시 부딪히던 문제.** watcher가 EMFILE이나 예산 초과로 자신을 해제하면 `forward`는 그 인스턴스를 집합에서 지우기만 했다. 지우기만 하면 다음 라운드가 대체 watcher를 승인하고, 그건 같은 트리를 걸어 같은 벽에 부딪히고, 다시 지워진다 — 디스크립터를 태우며 수렴하지 않는 재생성 루프다. 뒤늦게 발견된 승인 실패도 승인 실패이므로, 이제 같은 쿨다운을 받는다. 그동안 해당 워크트리는 heartbeat 폴링으로 돌아간다. 실측으로는 파일 5,000개까지 예산 안이라 감시가 유지되고(fd 5,015), 10,000개에서 반납이 일어나며(fd 9), 15,000개에서도 9로 고정된다 — 수정 전에는 6,000개 파일이 6,000개 디스크립터였고 상한이 없었다.
...
- **`gk sw`의 줄 마커가 "이 브랜치가 어디에 있나"를 말한다 — `★ ● ◌ ⊘ ○`.** 종전에는 로컬이면 전부 `●`였고, 로컬-원격 양쪽에 있는지 로컬에만 있는지는 UPSTREAM 컬럼의 `↑ origin/x` / `(local)`을 읽어야만 알 수 있었다. 그런데 그 컬럼은 터미널이 좁아지면 통째로 사라지는 컬럼이라, 정작 화면이 좁을 때 구분이 통째로 없어졌다. 마커는 어떤 폭에서도 살아남으므로 그 자리로 옮겼다: `★` 현재 브랜치, `●` 로컬·원격 양쪽, `◌` 로컬 전용(한 번도 push 안 됨), `⊘` upstream이 원격에서 사라짐, `○` 원격 전용. `●` 판정은 upstream 설정 여부만 보지 않는다 — `git switch -c` 후 `--set-upstream` 없이 push한 브랜치는 upstream이 비어 있어도 원격에 분명히 있으므로, 원격의 동명 ref 존재까지 확인한다(원격 목록은 이미 `r` 토글 카운트를 위해 읽고 있어서 git 호출은 늘지 않는다). gone에 `⊗`가 아니라 `⊘`를 쓴 이유는 `gk wt add` 피커가 `⊗`를 "다른 worktree가 이미 점유"에 쓰고 있어서다. 원 다섯 종류를 외우게 할 수는 없으므로 **부제 아래 흐린 한 줄로 범례**가 늘 붙는다 — 지금 목록에 실제로 있는 마커만 설명하고(원격 행이 숨겨져 있으면 `○` 항목도 없다), 터미널이 좁으면 마지막 항목을 `○ remot…`로 자르는 대신 짧은 표현(`★ here · ● both · ◌ local · ○ remote`)으로 갈아탄다. 상태가 한 종류뿐이면 구분할 게 없으므로 줄 자체가 사라진다.
- **피커 테이블이 색을 폭으로 세지 않는다 — 좁은 터미널에서 컬럼이 부당하게 날아가던 문제.** 컬럼 폭을 `runewidth.StringWidth`로 재고 있었는데, 이 함수는 ANSI 이스케이프를 **눈에 보이는 칸으로 센다**. `● mesh-project-sync ↑32 ↓5` 한 줄이 화면에서는 26칸인데 50칸으로 측정됐고, lipgloss는 그 50칸까지 실제 공백으로 채우니 브랜치 이름과 AGE 사이에 거대한 여백이 생기고, 반응형 컬럼 드롭은 "안 들어간다"고 판단해 UPSTREAM·HASH·WORKTREE를 차례로 버렸다 — 실제로는 전부 들어갈 폭이었는데도. 원인은 bubbles/table의 클리핑(`runewidth.Truncate`)도 ANSI를 모른다는 데 있었다: 폭을 정직하게 재면 이번엔 색 코드 한가운데가 잘려 색이 줄 전체로 번졌기 때문에, 폭을 부풀리는 쪽이 차악이었다. 그래서 테이블 렌더러를 ANSI를 아는 자체 구현으로 바꿨다(`internal/ui/table_render.go`) — 폭은 `lipgloss.Width`로 재고, 클리핑은 이스케이프를 통째로 보존한 채 보이는 칸 수로만 자르며 잘린 자리는 색을 닫는다. 그 결과 할당 폭 = 화면 점유 폭이 되어, 컬럼은 정말로 안 들어갈 때만 드롭된다. 커서 하이라이트·스크롤·`j/k`, `pgup/pgdn`, `g/G` 키는 종전 그대로다. 영향 범위는 `gk sw`뿐 아니라 같은 피커를 쓰는 `gk wt`, `gk branch clean`, PR/이슈 피커 전부다.
...
- **`gk commit`의 privacy gate가 문서 예제 키와 같은 값의 반복을 각각 세어, 진짜 자격증명이 0건인데 커밋을 막던 문제.** 임계값은 매치 개수를 셌다. 그래서 AWS가 공식 문서에 쓰라고 공개한 `AKIAIOSFODNN7EXAMPLE` 하나가 픽스처 여섯 군데에 있으면 여섯 건으로 계수됐고, 제보된 실제 사례에서는 그렇게 모인 11건(진짜 자격증명 0건)이 임계 10을 딱 하나 넘겨 abort됐다. **오탐이 임계 예산을 소진하면 진짜 secret이 가려진다** — 그게 이 버그의 실제 비용이다. 게다가 그 상태에서 사용자에게 남는 선택지는 `--skip-privacy`를 습관화하거나 `max_secrets`를 올리는 것뿐인데, 둘 다 앞으로의 모든 커밋에서 게이트를 약화시킨다. 특히 **redaction 기능을 가진 프로젝트일수록 심하다**: 그게 동작하는지 검증하려면 테스트에 secret처럼 생긴 픽스처를 의도적으로 넣어야 하기 때문이다. 이제 임계는 **구별되는 실제 후보의 수**를 센다 — 같은 값이 N번 나오는 건 "N개의 secret"이 아니라 "1개가 N군데"이고, 공개된 예제 키는 애초에 secret이 아니다. 예제·자리표시자 판정은 `gk push`/`gk ship`의 스캐너가 이미 쓰던 것을 그대로 공유하므로(`internal/secrets`에서 export), 같은 값을 한쪽은 통과시키고 다른 쪽은 막던 비대칭도 함께 사라진다. **redaction 자체는 전부 그대로다**: 게이트의 본래 임무는 원문이 원격 provider에 닿지 않게 하는 것이고, 자리표시자 판정은 값 옆의 단어까지 읽는 넓은 휴리스틱이라 "확실히 자격증명이 아님"보다 느슨하다. 공개된 예제 키를 가려서 잃는 건 없지만 "example" 옆에 앉아 있던 진짜 키를 흘리면 잃는 게 크다 — 그래서 **가리되 세지 않는다**. 리포트도 그 차이를 밝힌다: 제외된 항목마다 `(not counted: placeholder | duplicate)`가 붙고, 목록 개수와 임계 숫자가 다른 이유를 한 줄로 설명한다. PEM 블록은 자리표시자 판정에서 면제된다 — 스캐너의 `private-key` 예외와 같은 이유로, 헤더 자체가 모호하지 않은 서명이기 때문이다.
...
- **`gk wt`의 add new에서 기존 브랜치를 목록으로 고른다 — 이름을 기억해 타이핑하던 단계 제거.** `[x] existing`을 골라도 브랜치명을 직접 입력해야 해서, "예전에 작업하던 게 뭐였지"에는 답할 수 없는 폼이었다. 이제 브랜치 picker가 열리고 **최근 커밋순**으로 정렬된다 — `gk sw`가 로컬을 알파벳순으로 두는 것과 의도적으로 다르다. sw는 "아는 브랜치로 이동"이라 알파벳 스캔이 맞지만, 이 목록은 "뭘 하고 있었지"에 답해야 하고 그건 시간순으로만 드러난다. 리모트 브랜치는 기본 목록에서 빠져 있다가 `ctrl+r`로 펼치거나 **필터를 입력하는 순간 합류**하므로, 이름으로 찾을 때 스코프를 먼저 넓혀야 한다는 걸 알 필요가 없다. 리모트를 고르면 worktree를 만드는 같은 명령에서 `--track`으로 로컬 브랜치와 upstream이 함께 생긴다(git의 DWIM 대신 명시적 — 여러 리모트가 동명 브랜치를 들고 있으면 모호해진다). 점유 판정에는 **현재 worktree도 포함**된다: 지금 서 있는 브랜치도 두 번째 체크아웃은 git이 거부하므로 다른 worktree가 문 브랜치와 똑같이 쓸 수 없다. 점유된 줄은 숨기지 않고 `⊗` 마커로 어느 worktree가 물고 있는지 밝힌 뒤 선택만 거부하므로, 찾던 브랜치가 목록에서 사라져 당황할 일이 없다. create 토글이 첫 스텝으로 올라갔고(서로 다른 두 흐름을 가르는 선택이라서), worktree 이름은 고른 브랜치에서 제안된다.

### Fixed

- **필터를 입력하는 동안 picker의 액션 단축키가 통째로 먹히지 않던 문제.** `/`로 필터를 켜면 이후 모든 글자가 필터 텍스트로 들어가서, `r`(remotes)·`f`(fetch)·`d`(delete)·`n`(new)이 `esc`를 먼저 누르기 전에는 동작하지 않았다. 사용자 입장에선 "키가 안 먹는다"로 보였고, 필터를 친 뒤 `r`을 누르면 필터가 오염돼 목록이 빈 것처럼 되기까지 했다. 이제 각 액션에 `ctrl` 별칭이 붙어 필터 입력 중에도 발화한다 — `ctrl+r`/`ctrl+f`/`ctrl+d`/`ctrl+n`, `gk wt` picker의 global 토글은 `ctrl+g`. 별칭은 필터 입력창이 키를 보기 전에 가로채므로 같은 조합의 편집용 바인딩(`ctrl+f` 커서 이동 등)보다 우선한다. 평문 글자는 별칭으로 쓸 수 없다 — 그렇게 하면 그 글자를 필터에 입력할 방법이 사라지기 때문이다. 필터 모드의 헬프바도 쓸 수 있는 별칭을 직접 나열하도록 바뀌었다.

### Changed

- **`gk sw` picker의 `D`(force delete) 키 제거 — 확인창에서 승격하는 방식으로 대체.** 터미널은 `ctrl+D`를 `ctrl+d`와 다른 바이트로 인코딩하지 못하므로, 단축키를 `ctrl` 별칭 체계로 옮기면서 대소문자로 갈린 키는 유지할 수 없었다. 이제 `d`로 지우려는 브랜치를 git이 거부하면(미머지·protected·기본 브랜치) 그 자리에서 force 확인창이 뜨고 거부 사유가 그대로 문구에 실린다. 이미 "이걸 지우겠다"고 누른 사용자에게 다른 키를 찾아 다시 누르라고 안내하는 것보다, 결과를 묻는 쪽이 맞다고 판단했다. 현재 브랜치나 리모트 행처럼 git이 어차피 거부할 대상은 승격되지 않는다.
...
- **`gk find <query>` — 히스토리 검색을 한 번의 호출로.** 세션 감사가 남은 raw git 중 가장 큰 덩어리로 지목한 것이 히스토리 사냥이었다. 핵심은 "`gk log`에 `--grep`이 없다"가 아니다 — 그 플래그를 그냥 추가하는 건 1:1 스왑이라 턴을 하나도 줄이지 못한다. 진짜 비용은 **어떤 질의가 맞을지 모른다는 것**이고, 그래서 에이전트는 추측마다 한 턴씩 낸다: `git log --grep`(빈손) → `git log -S`(빈손) → `git log -- <경로>`(찾음). 실제 세션에 남아 있는 그대로다. 이제 `gk find`가 세 검색을 **동시에** 돌린다 — 커밋 **메시지**(`--grep`), 바뀐 **내용**(`-S` pickaxe), **경로** — 그것도 현재 브랜치가 아니라 **모든 ref**에 대해. 각 결과에는 **무엇이 매치됐는지**가 붙어서, "메시지엔 한마디도 없는데 코드가 여기서 바뀌었다"가 한눈에 보인다(두 번째 질의로 다시 캐낼 필요가 없다). 두 가지 이상으로 매치된 커밋이 최상위로 랭크된다 — 가장 강한 신호이기 때문이다. `--path`/`--since`/`--author`/`--ref`로 좁히고, 큰 저장소에서 가장 느린 pickaxe는 `--no-content`로 끌 수 있다. 실패한 모드는 `failed`에 실려 나머지 결과와 함께 반환된다 — 부분 답이 무답보다 낫지만, 완전한 답인 척해서는 안 된다("못 찾음"을 "히스토리에 없음"으로 읽게 만든다). 감사에 `find` collapse group을 배선해 효과를 실측한다: 로컬 코퍼스(세션 3307개)에서 **33턴 절감**으로 측정됐다. **답하지 않는 것도 명시한다** — `git log A..B`("A에 없고 B에 있는 것")는 검색이 아니라 범위 비교라서 `gk find`가 아니라 `gk log --ahead/--behind --base`의 영역이고, 감사에서도 별도 갭(`raw-range-compare`)으로 남긴다. ([docs/commands.md](docs/commands.md#gk-find-alias-search))

### Fixed

- **`gk session audit`가 `gk context`로 답할 수 없는 턴까지 "context가 절감해줄 턴"으로 계상하던 문제 — 히스토리 검색·브랜치 조사를 분리.** 감사는 `git log`·`git branch`의 **모든** 읽기 전용 형태를 `raw-context-probes`(= "gk context가 커버함")로 찍었다. 하지만 `gk context`는 **"지금 내가 어디 있나"**(현재 브랜치·upstream·ahead/behind·dirty·최근 커밋)에 답하는 명령이지, **"어느 커밋이 X를 넣었나"**(`git log --grep`, `-S` pickaxe, `--follow`, 경로 한정 log, ref 범위 비교)나 **"어떤 브랜치가 main에 합쳐졌나"**(`git branch -a --merged`)에는 답하지 못한다. 그 결과 두 가지가 동시에 망가져 있었다 — turn 지표가 gk가 절대 절감할 수 없는 턴을 절감분으로 계상했고(실측 코퍼스에서 context 그룹 **143턴 중 86턴이 허수**), 인라인 힌트·PreToolUse 훅이 에이전트를 **자기 질문에 답하지 못하는 명령으로** 밀어붙였다. 이제 셋으로 가른다: 진짜 상태 조회만 `raw-context-probes`(3481 → 1757), 브랜치 조사는 **`gk branch list`**가 커버(`--merged/--unmerged/--gone/--stale`, `--json`을 갖췄다 — 신규 `raw-branch-list` 490건), 히스토리 검색은 **`raw-history-search`(gap, 666건)** — gk에 대응 동사가 아예 없기 때문이다(`gk log`에는 `--grep`도 `-S`도 pathspec도 없다). 이건 채택률 문제가 아니라 **기능 공백**이고, 그렇게 읽혀야 한다. 아울러 **대체재가 없는 kind는 힌트가 침묵한다** — 종전엔 gap kind가 `use  instead of raw git`처럼 빈 추천을 뱉었다. 알려진 한계: `origin/main..HEAD` 범위는 사실 `gk log --ahead`로 답할 수 있지만, 그 범위가 마침 그 브랜치의 upstream인지는 저장소 상태 없이 구분할 수 없어(같은 이유로 `isRawUnstage`가 pathspec-only 형태를 세지 않는다) 범위는 전부 gap으로 센다. turn rate가 5.8% → 3.1%로 떨어지는데, 이는 후퇴가 아니라 허수가 걷힌 것이다 — `--record` 이력에서 이 시점의 계단식 하락은 채택률 하락으로 읽지 말 것.

- **`gk session audit`가 `git reset --hard <ref>`와 `git fsck` 복구 탐색을 "gk 동사 없음"으로 오분류하던 문제.** 분류기가 작성된 뒤에 `gk reset --to <ref>`와 `gk restore --lost`가 생겼는데 매핑이 따라가지 못해, 감사는 이미 커버되는 명령을 coverage gap으로 보고했다 — 로드맵이 "만들어야 할 것" 목록에 이미 있는 기능을 올리게 된다. 이제 **대상(target) 기준으로** 매핑한다: `git reset --hard <ref>` → `gk reset --to <ref>`(같은 목적지로 되돌리되, 먼저 target을 fetch하고 확인을 받는다 — `-y`로 건너뛴다. 백업 ref를 쓰지는 않으므로 되돌아갈 길은 reflog뿐이고, 그게 `gk undo`다), `git fsck --lost-found/--unreachable/--dangling` → `gk restore --lost`(그 fsck 탐색을 감싼 것이 이 명령이다). 실측 코퍼스(세션 3307개)에서 uncovered 90 → 73건. **의도적으로 매핑하지 않은 것**도 함께 고정했다 — 잘못된 추천은 작업을 날리기 때문이다: 인자 없는 `git reset --hard`는 HEAD에서 작업트리를 버리지만 인자 없는 `gk reset`은 **upstream 원격으로** 되돌리므로(철자만 같고 목적지가 다르다) 매핑하지 않는다. `--soft`도 마찬가지다 — gk의 유일한 "커밋만 취소하고 작업은 보존" 경로인 `gk undo`는 대화형 reflog 피커라 에이전트가 몰 수 없다. `git checkout -- <path>`/`git restore <path>`(경로 단위 폐기)는 `gk wipe`가 작업트리 전체를 대상으로 하고 추적 안 되는 파일까지 지우므로 **여전히 진짜 갭**이며, 그렇게 남겨 둔다. 분류기는 인라인 `gk hint`와 단일 소스라 감사·훅·turn 지표가 함께 정확해진다.

## [0.121.0] - 2026-07-13

### Added

- **`gk watch`가 "지금 얼마나 쓰이고 있나"를 숫자로 답한다 — 3계층 diffstat + 세션 Δ.** 대시보드는 어떤 파일이 바뀌는지는 보여줬지만 **얼마나** 바뀌는지는 안 보여줬다 — dirty 카운트(`U22 ?5`)는 파일을 상태별로 셀 뿐이고, 피드는 개별 이벤트라 총량이 안 잡힌다. 이제 세 계층에서 미커밋 diffstat을 읽는다: worktree 행(`+31 −3`), repo 그룹 행(`7 files  +31 −3` — 접힌 repo도 총량은 보고한다), 헤더 전체 합계. 추가 git 호출은 없다 — 폴 때마다 이미 돌던 스캔의 파일별 시그니처를 합산할 뿐이다. 헤더에는 **Δ**도 붙는다(`Δ +3,142 −210 over 41 files · 17m`): watch를 켠 뒤 실제로 발생한 변경의 누적으로, 에이전트가 커밋하면 0으로 리셋되는 절대량과 달리 사라지지 않는다 — "지금 미커밋이 얼마나 쌓였나"와 "켜둔 동안 얼마나 작업됐나"는 다른 질문이고 둘 다 볼 값이 있다. Δ는 폴 간 **양(+)의 증분**만 더한다: 시그니처의 `+/−`는 HEAD 대비 누적치라 피드 줄을 그냥 합산하면 재수정된 파일이 매 폴마다 중복 계상되고, 커밋으로 카운트가 줄어드는 건 "음수 작업"이 아니라 기준선 리셋이므로 0으로 친다. watch 시작 시점에 이미 dirty였던 것은 피드와 동일하게 조용한 baseline이다. `--feed-stats=false`면 파일 수만 남고 `+/−`는 빠지며(diff 호출을 안 하므로), 좁은 터미널에서는 Δ → 행 컬럼 순으로 접힌다. `--json` 엔트리에도 `files`/`added`/`removed`가 실린다. ([docs/commands.md](docs/commands.md#gk-watch))

### Fixed

- **`gk watch`의 화면이 watcher 재구성 때 멈추지 않는다 — 파일시스템 walk를 뮤텍스 밖으로.** worktree 하나가 깨어나 watcher를 새로 받을 때마다 대시보드가 잠깐씩 굳었다(worktree당 약 30ms, fleet 전체가 한꺼번에 재-grant되면 약 600ms). `fleetWatchSet.sync`가 `ws.mu`를 **본문 내내** 쥔 채로 `newFSWatcher`를 돌렸기 때문이다 — watcher 하나를 세우는 일은 `git rev-parse`/`ls-files` fork 2회에 작업트리 전체 재귀 walk(kqueue에서는 파일마다 디스크립터)라 결코 가볍지 않은데, `View()`는 매 프레임 `hasAny()`로 **같은 뮤텍스**를 잡으러 오고 `sync`는 `tea.Cmd` goroutine에서 돈다. 렌더가 walk 뒤에 줄을 선 것이다. 이제 sync는 락 안에서 **계획만** 세우고, walk와 teardown은 락을 풀고 수행한 뒤, 다시 락을 잡아 **설치**한다. 락을 푼 사이 set이 움직일 수 있으므로 설치 단계에서 이를 처리한다: 대시보드가 종료됐거나 동시 sync가 같은 경로를 먼저 가져갔다면 방금 만든 watcher를 **닫는다** — 디스크립터를 쥔 채 버리는 누수만은 예산이 흡수하지 못한다. (실측: walk 39ms 동안 렌더 읽기의 최악 지연이 39ms → 0.08ms.)

- **`gk watch`가 CPU를 태우지 않는다 — fs 이벤트 poll에 rate limit, 그리고 매 폴 반복되던 git 호출 제거 (17-repo 기준 122% → 7% CPU).** 에이전트가 붙어 일하는 fleet을 켜 두면 `gk watch`가 코어 하나를 통째로 물고 늘어졌다(실측: 21개 worktree에서 116% CPU가 14분간 지속, 스레드 52개, fd 1487개). 원인은 백프레셔가 **동시 폴 2개만** 막고 **폴 빈도는 전혀** 막지 않았다는 것 — 폴이 도는 0.35초 사이에 어느 worktree든 파일이 하나 바뀌면 `fsPending`이 서고, 그 폴이 끝나는 즉시 **쿨다운 없이** 다음 폴이 시작됐다. 한 worktree가 계속 저장되기만 하면 전체 fleet 스캔이 무한히 연달아 도는 구조였고, 폴 한 번이 git 프로세스 251개에 CPU 1.8초였다. 이제 폴 **시작 간격**이 설정된 `--interval`(단일 2s / 멀티 5s) 이상으로 강제된다 — 이벤트는 버려지지 않고 쿨다운 뒤로 미뤄질 뿐이며(`deferred`), 이벤트가 사 주는 것은 "폴을 더 자주"가 아니라 "타이머를 기다리지 않는 즉시성"이다. 수동 `r`은 rate limit을 무시한다(키 입력은 이 제한이 막으려는 폭주가 아니다). 폴 자체의 원가도 함께 깎았다: worktree마다 gitdir를 다시 묻던 `rev-parse` 2회를 memoize하고(79 → 6), repo마다 default branch를 **두 번** 묻던 중복을 제거하고(`symbolic-ref` 34 → 17), fork point의 `merge-base`를 **두 커밋 tip을 키로** 캐시했다(33 → 13 — 내용 기반이라 tip이 움직이면 다른 키가 되므로 stale이 원리적으로 불가능하다). 폴당 git 호출 251 → 137, idle CPU도 47% → 5%.

- **`gk watch`가 터미널을 다 쓴다 — 80칼럼 섬 + 8줄 피드 상한 제거.** 넓은 터미널에서 대시보드가 좌상단 80×15에만 그려지고 나머지는 빈 화면이었다. 원인은 렌더러 네 곳(`renderFleet`/`renderFleetGrouped`/`renderFleetFeed`/zoom breadcrumb)에 복붙된 `if width <= 0 || width > 120 { width = 80 }` — 폭이 120을 넘으면 **클램프가 아니라 80으로 폴백**해, 터미널이 넓을수록 오히려 더 좁게 그려졌다. 이제 `fleetWidth`가 실제 폭을 그대로 쓰고(하한만 유지), 남는 가로 공간은 `fleetColumns`가 정보가 실린 두 컬럼 — 브랜치, 마지막 변경 파일 — 에 배분한다: `ContentView.s…`로 잘리던 경로가 `app/Sources/SpaceMeshApp/ContentView.swift`로 온전히 보이고(좁아지면 basename이 아니라 식별에 필요한 꼬리부터 남긴다), 디테일 패널은 실측 폭(`lipgloss.Width`)만큼만 테이블에서 가져간다. 세로도 같은 문제였다 — 피드 페인이 8줄로 고정돼 48줄 터미널에서 30줄이 놀았다. 이제 대시보드가 실제로 그린 높이(디테일 패널이 테이블보다 길 수 있으므로 행 수가 아니라 렌더 결과 기준)를 뺀 나머지를 피드가 채우고, 키바는 화면 맨 아래에 고정된다.

- **`gk sync`가 브랜치의 fork 부모(gk-parent)를 base로 인정한다.** develop에서 만든 worktree에서 `gk sync`를 돌려도 base가 항상 main으로 잡히던 문제 — sync의 base 해석이 `DefaultBranch`(= origin/HEAD = main)만 보고 브랜치별 `gk-parent` 메타데이터를 무시했기 때문이다. branchparent가 부모를 기록해 두는데도(`gk wt add`가 자동 기록) status는 그 부모 기준으로 비교하는 반면 sync만 안 읽어, "develop에서 판 wt인데 왜 main?"이 정확히 발생했다. 이제 sync의 base 우선순위를 **명시적 `--base` > 기록된 `gk-parent` > config base > origin/HEAD**로 맞춰, develop에서 컷한 브랜치는 develop 위로 rebase한다(status의 base 해석과 동형). 부모 조회를 `DefaultBranch`보다 먼저 하므로 remote가 없는 저장소에서도 부모가 있으면 sync가 동작한다. sync는 히스토리를 재작성하는 경로라 `land`의 머지 대상 결정과 같은 이유로 `Resolver.ExplicitOnly()`를 써 reflog 추론은 base로 채택하지 않는다 — 명시적 `gk-parent`가 없으면 기존대로 origin/HEAD로 폴백한다.
...
- **`gk chat`에 도구 4종이 늘고 시스템 컨텍스트가 깊어졌다 — `git_status`/`git_snapshot_list`/`git_snapshot_diff`/`git_context`, `REPO_MAP`.** 기존 7종(log/show/diff/blame/grep/file_read/file_list)만으로는 "지금 dirty한가", "스냅샷 있나", "리베이스 중인가" 같은 질문에 log나 grep을 여러 번 우회 호출해야 했고, REPL 세션이 길어지면 세션 시작 시 한 번 수집한 `REPO_CONTEXT`가 낡아도 다시 확인할 길이 없었다. 이제 `git_status`(staged/modified/conflict/untracked/stash/진행 중인 rebase·merge·cherry-pick·revert를 구조화 요약), `git_snapshot_list`(`refs/wip` reflog 나열 — `git stash`와는 별개인 gk의 안전망), `git_snapshot_diff`(스냅샷 ↔ 워킹트리, digest 기본·`raw`로 전체 패치), `git_context`(시스템 프롬프트의 `REPO_CONTEXT`와 동일한 스냅샷을 그 자리에서 재조회 — 대화 중간에 브랜치를 바꾸거나 rebase를 시작했을 때 드리프트를 잡는 용도)가 추가됐다. `REPO_CONTEXT` 자체도 `gk context`를 그대로 재사용해 브랜치/upstream/ahead-behind/dirty 요약/진행 중 작업(재개·중단 명령 포함)/base drift/최신 태그/연결된 worktree 수까지 풍부해졌다. `ai.chat.auto_context: true`(기본 off)를 켜면 `git ls-files` 기반 디렉터리 트리를 `REPO_MAP`으로 시스템 프롬프트에 주입해(깊이 3·파일 300개 상한, aider의 repo-map을 구조 전용으로 축소한 버전) "이 프로젝트 구조가 어떻게 되지?" 류 질문이 `file_list`/`git_grep` 왕복을 소비하지 않게 한다. 새 도구들도 기존 read-only 샌드박스·redact-before-persist 파이프라인을 그대로 통과한다.
- **`gk chat`가 실시간으로 답하고, 긴 대화를 스스로 접고, 더 잘 버틴다.** 답변이 다 만들어질 때까지 화면이 멈춰 있어 긴 답일수록 대기가 길게 느껴졌고, 히스토리가 쌓여도 사용자가 직접 관리할 수단이 없었으며, 세션 시작 실패나 라운드 타임아웃이 종종 세션 전체를 끝장냈다. ① **스트리밍**: 텍스트 응답이 토큰 단위로 실시간 출력되고, 도구를 호출하는 라운드는(최종 텍스트가 아직 없으므로) 자동으로 non-stream 요청으로 폴백하며, JSON/agent 모드(`--json`/`GK_AGENT=1`)는 스트리밍 자체가 꺼진다(envelope는 완성된 답 전체가 필요하므로). ② **`/tokens`**: 시스템 프롬프트·히스토리·도구 결과의 구성비를 문자 수와 근사 토큰 수로 보여주고 `ai.chat.history_budget`(기본 32768) 대비 잔량을 보고하며, 매 턴이 끝날 때 사용량이 80%를 넘으면 같은 기준으로 자동 경고한다. ③ **`/compact`**: 가장 최근 2턴은 원문 그대로 남기고(지금 진행 중인 조사 상태를 손실 있는 요약으로 바꿔버리지 않도록) 그 이전 턴 전체를 provider 요약 호출 1회로 접는다 — 요약 결과가 세션 파일에 기록되므로 나중에 `--continue`로 replay해도 압축된 히스토리 그대로 재현된다. ④ **안정성**: 세션의 첫 라운드(아직 vendor tool-call ID가 하나도 커밋되지 않은 시점)가 실패하면 폴백 체인의 다음 provider로 자동 재시작하고(그 이후 라운드의 실패는 provider를 바꾸지 않는다 — 이미 커밋된 tool-call ID 공간을 깨뜨리지 않기 위해), `ai.chat.round_timeout`이 개별 HTTP 시도 상한과 재시도 루프 예산 양쪽의 바닥값이 되고(아래 cross-vendor 교정 ⑫ 참조 — 이 상한은 응답 본문 읽기까지 포함하므로, SSE 스트림 하나가 곧 답변 전체인 chat에서 30초를 넘기는 정상 답변이 잘리지 않는다), 멈춘 스트림이 라운드 예산을 독식하지 못하도록 스트리밍 *시도*에만 별도의 짧은 서브데드라인을 둬 폴백에 시간을 남기며, 도구 스키마를 위반한 응답(존재하지 않는 도구, 필수 인자 누락)은 1회 재프롬프트한 뒤에도 여전히 깨져 있으면 정상적인 dispatch 에러로 넘어간다.
- **agent 모드의 chat 실패 상태가 정확해지고, `gk doctor`가 provider별 chat 가용성을 알려준다.** `GK_AGENT=1`에서 `gk chat`의 실패는 전부 뭉뚱그려진 `error`였다. 이제 라운드 한도 초과(`ai.chat.max_tool_rounds`)는 `state: blocked` + 라운드 상향/더 빠른 `--model` remedies로, 세션 시작 폴백 체인이 전부 실패한 경우도 `state: blocked` + `gk doctor` remedy로 분류되고, 라운드 타임아웃은 `state: error` + 동일 remedies로(재시도만으로는 해결되지 않으므로), Ctrl-C/취소는 remedy 없는 `state: error`로 구분된다. `gk doctor`(`--ai`/`--verbose`)는 provider마다 `ai chat: <provider>` 행을 추가해 tool-calling 지원 여부와 인증 상태를 `gk ask`류 API 가용성 행과 별도로 보고한다.

### Fixed

- **3차 cross-vendor 패널이 2차 수정 자체가 낳은 회귀 2건(high)을 잡았다.** 같은 3모델(claude-opus-4-8/codex/cursor)에 2차 수정 반영 diff를 재리뷰시킨 결과다. ① **스트리밍 하이브리드의 폴백 예산 분할(`streamAttemptShare=0.6`)이 정상적인 긴 답변을 죽였다**: hung 스트림이 라운드 예산을 독식하는 것을 막으려 스트리밍 시도에 라운드 잔여의 60% 상한을 뒀는데, 총-시간 상한은 hung(무응답)과 slow-but-progressing(정상적으로 오래 걸리는 답변)을 구분하지 못한다 — 생성이 60%를 넘긴 답변은 잘린 뒤 남은 40%로 처음부터 재생성돼 사실상 영구 실패했다(직전 ⑫ Timeout floor가 없애려던 truncation의 재발). 이제 스트리밍 시도는 라운드 컨텍스트 전체를 쓴다. hung 스트림의 상한은 다른 모든 요청과 동일한 per-attempt HTTP Timeout(round_timeout floor)이며, 그 경우 non-stream 폴백도 같은 dead endpoint라 어차피 실패하므로 별도 예산을 남길 실익이 없다. ② **단일 provider의 first-round 타임아웃이 `no_provider`(blocked)로 오분류됐다**: 세션 시작 폴백이 첫 라운드 실패를 전부 `errChatFallbackExhausted`로 감싸면서, 폴백할 다음 provider가 없는 단일 후보의 round_timeout까지 "provider 없음"으로 보고됐다. 이제 후보가 하나면 원래 에러(engine이 `ErrFirstRoundFailed` 안에 이중 `%w`로 보존한 deadline)를 그대로 반환해 `round_timeout`으로 정확히 분류한다. 아울러 2차가 `-z` 파싱으로 전환하며 무력화된 `git_status`의 rename deny 필터(옛 `" -> "` 분리는 `-z`에서 발화하지 않음)를 origin 필드 기반으로 되살려, denied 경로에서 나온 rename이 destination만으로 노출되던 약한 오라클도 막았다. ③ **deny-aware dirty 재계산 실패가 REPO_CONTEXT를 "clean"으로 위조했다**: 2차에서 REPO_CONTEXT의 dirty 카운트를 deny-필터된 `countChatDirty`로 재계산하도록 바꿨는데, 그 `git status` 호출이 일시 실패하면 zero(clean)를 반환해 collector가 이미 계산한 진짜 dirty를 덮어썼다 — F6(수집 실패를 사실로 위장하지 않기)이 고친 것과 같은 클래스를 재도입한 것이다. 이제 재계산 실패는 카운트를 UNKNOWN으로 취급해 REPO_CONTEXT 전체를 강등하고, 다시는 "status가 딸꾹질했으니 작업트리가 깨끗하다"고 단언하지 않는다.
- **`gk chat`의 실패·압축·스트리밍 경로를 cross-vendor 패널이 다시 교정했다 — logic 렌즈 confirmed 13건.** 같은 4개 벤더 모델에 logic 렌즈로 재리뷰시켜 나온 결함들이다. ① **agent 모드가 실패를 성공으로 보고했다**: `chatOneShotFailure`가 분류하지 못한 실패(provider 500, 인증 오류, 파싱 오류)는 `failure`를 비워 뒀고, 빈 값은 봉투의 기본값 `state:"ok"`로 흘러 stdout이 `ok:true` + 빈 답변을 내보냈다 — stderr의 에러 봉투와 정면으로 어긋나는, `state`로 분기하라는 gk 자신의 규약 위반이다. 이제 미분류 실패도 `state:"error"`가 된다. ② `classifyChatTurnError`가 substring 휴리스틱(`isDeadlineErr`)을 sentinel 검사(`errors.Is(errChatFallbackExhausted)`)보다 먼저 돌려, 모든 provider가 타임아웃으로 실패한 경우를 `no_provider`(blocked)가 아니라 `round_timeout`으로 오분류했다 — 정확한 검사를 앞으로 옮겼다. ③ 첫 라운드에서 Ctrl-C를 누르면 취소가 `ErrFirstRoundFailed`로 감싸여 같은 질문이 다음 provider로 재발사됐다 — 사용자 취소는 폴백 대상이 아니다. ④ 첫 턴이 **모든** 후보에서 실패해도 REPL이 폴백 게이트를 닫아, 히스토리가 여전히 비어 있는데도 이후 질문이 마지막 실패 provider에 고정됐다. ⑤ **`/compact`가 Anthropic 세션을 통째로 깼다**: 폴딩 결과의 첫 메시지가 assistant 역할이었는데 Messages API는 첫 메시지가 user여야 한다 — 이제 요약 앞에 합성 user 인트로를 두어 역할 교차를 유지하고, 어댑터에도 이 불변식을 강제하는 가드를 넣어 다음번엔 조용한 400 대신 명확한 에러로 실패한다. ⑥ compact 후의 hard trim이 세션에 기록되지 않아 `--continue` replay가 live 히스토리와 갈렸다 — trim 예산을 compact 레코드에 함께 적고 replay가 같은 트림을 재적용한다. ⑦ `git_snapshot_diff`가 스냅샷을 현재 *인덱스*와 비교해, 스냅샷 시점에 untracked였던 파일을 "삭제됨"으로 오보고했다 — `gk snapshot diff`와 같이 임시 인덱스로 워킹트리 트리를 만들어 tree-to-tree diff를 뜬다. ⑧ `git_status`가 untracked 파일만 있는 작업트리를 `clean:true`(+`untracked_count:3`)라는 자기모순으로 보고했다 — 도구 출력의 `clean`은 untracked를 포함하도록 재정의하고 스키마에 명시했다(gk 내부의 `DirtyFlags` 의미는 불변). ⑨ 손상된 SSE 청크를 조용히 건너뛰어 불완전한 텍스트가 성공한 답변으로 확정될 수 있었다 — 파싱 불가 이벤트는 스트림 실패로 취급해 non-stream 폴백을 탄다(주석·keep-alive·`[DONE]` 같은 정상 SSE 요소는 그대로 처리). ⑩ 멈춘 스트림이 라운드 예산을 전부 소진해 폴백에 남는 시간이 없었다 — 스트리밍 시도는 남은 예산의 60%만 쓰고 나머지를 폴백에 남긴다. ⑪ `collectChatContext`가 수집 실패를 삼켜 `REPO_CONTEXT`가 "이름 없는 브랜치, clean, up-to-date"라고 *단언*했다 — 이제 실패를 사실로 위장하지 않고 강등한다. ⑫ **`ai.<provider>.timeout`이 chat 라운드를 잘랐다**: 직전 수정이 `minTimeout` 해킹을 걷어내며 per-attempt HTTP 상한을 provider 기본값(30s)에 남겨 뒀는데, `http.Client.Timeout`은 응답 본문 읽기까지 포함하므로 SSE 스트림 하나가 곧 답변 전체인 chat에서는 30초를 넘기는 정상 답변이 잘렸다 — `round_timeout`을 개별 시도 상한의 바닥값으로 끌어올렸다. `RetryBudget`은 여전히 별도 필드지만(다른 명령들은 시도 상한을 예산보다 훨씬 작게 둔다) chat 경로에선 둘 다 `round_timeout`으로 같아지고, 이 경로에서 hung 시도가 라운드를 통째로 잡아먹는 것은 타임아웃 총량이 아니라 스트리밍 시도에 걸린 서브데드라인(⑩)이 막는다.
- **`gk chat`의 새 기능들이 deny_paths 경계를 넘지 않는다 — cross-vendor 패널이 잡은 5건.** 이번 사이클의 diff를 4개 벤더 모델(claude·codex·agy·cursor)에 security 렌즈로 교차 리뷰시킨 결과를 반영했다. ① `REPO_MAP`(`ai.chat.auto_context`)이 `git ls-files` 출력을 필터 없이 렌더링해, `file_list`와 `git_status`가 이름조차 내주지 않는 deny 경로를 트리에 그대로 열거했다(모델 3개가 독립적으로 지적) — redaction은 시크릿 *값*을 지우지 트리의 *파일명*을 지우지 않으므로, 이제 트리를 만들기 전에 도구들이 쓰는 것과 같은 deny 목록으로 경로를 거른다(자식이 전부 denied면 부모 디렉터리도 나타나지 않는다). ② `git_status`의 `clean`/`staged`/`modified`/`conflict` 플래그가 필터 *이전* porcelain에서 계산돼, `changed[]`가 이름을 감춰도 `clean:false`가 "denied 파일이 바뀌었다"는 오라클로 남았다 — 이제 필터된 엔트리에서 다시 계산한다. ③ SSE 파서에 총량 상한이 없어 짧고 정상적인 이벤트를 끝없이 보내는 엔드포인트가 라운드 데드라인까지 누적 버퍼를 키울 수 있었다(Scanner의 4 MiB 한도는 *한 줄*만 제한한다) — 16 MiB 상한을 넘기면 스트림을 버리고 기존 non-stream 폴백을 탄다. ④ `--session <id>`와 `last-session` 포인터의 내용이 검증 없이 `sessions/` 경로에 결합돼 `..`로 디렉터리를 벗어난 `.jsonl` 파일에 append할 수 있었다 — id 문자셋을 검증한다. ⑤ `/compact`가 대화 트랜스크립트(=repo가 통제하는 도구 출력)를 fence 없이 요약기에 넘겨, 인젝션이 요약을 조종하면 그 요약이 assistant 턴으로 히스토리에 되돌아왔다 — `REPO_CONTEXT`/`REPO_MAP`과 동일하게 `WrapUntrusted` fence로 감싼다.
