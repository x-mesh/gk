...
| `gk switch [name]` | `gk sw` | 브랜치 전환; `--fetch`로 remote branch를 먼저 갱신, `-m`/`--main`으로 main 이동, `-d`/`--develop`으로 develop |
...
| `gk worktree init [path]` | | `.gk.yaml`의 `worktree.init`로 worktree의 gitignore된 상태 복원: `link`(`.env` 등 시크릿 symlink), `copy`(worktree별 파일), `run`(`npm ci`, `uv sync`). 멱등이라 셋업 재시도로도 사용. 설정이 없으면 패키지 매니페스트를 감지해 블록을 제안(`--save`로 저장, `--dry-run`으로 미리보기) |
