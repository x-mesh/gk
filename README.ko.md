...
| `gk chat` | | 저장소와 대화 — 모델이 읽기 전용 도구(git log/show/diff/blame/grep + 파일 읽기/목록)를 스스로 호출해 근거를 찾아가며 답하는 에이전틱 루프. 모든 도구 호출이 한 줄씩 표시되고, 답변은 실제로 본 SHA와 `파일:라인`을 인용한다. REPL(`gk chat`) · one-shot(`gk chat "질문"`) · `--continue`(세션은 `.git/gk-chat/`에 저장). 샌드박스(repo 밖·symlink 탈출·.git·submodule 차단, deny_paths는 과거 커밋 출력에도 적용, 저장 전 redaction)와 상한(턴당 15라운드/192KB, 상한은 global config 전용)이 강제된다. tool-calling HTTP 프로바이더(anthropic/openai/groq/nvidia) 필요 |
...
| 명령어 | 설명 |
|---|---|
| `gk push` | 가드된 push: 시크릿 스캔 + 보호 브랜치 적용; `--force`는 `--force-with-lease`로 라우팅; `-n`/`--skip-scan`으로 시크릿 스캔 생략 |
