...
| `gk forget [path...]` | | 전체 git history에서 경로 제거 (`git filter-repo` 위임). 인자 없으면 `.gitignore`로 가린 tracked 파일을 자동 감지. rewrite 전에 backup ref `refs/gk/forget-backup/<branch>/<unix>` (`gk timemachine list`에 함께 표시) + `.git/gk/` 아래 평문 manifest 작성. `--analyze`는 rewrite 없이 path별 unique blob 수와 총 바이트를 보고, `--keep <glob>`은 forget 대상에서 제외할 경로를 받습니다 (`filepath.Match` 문법, 반복 가능). `--dry-run`/`--yes` 지원. `git-filter-repo` 설치 필요 |
