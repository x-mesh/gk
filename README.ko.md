...
| `gk forget [path...]` | | 전체 git history에서 경로 제거 (`git filter-repo` 위임). 인자 없으면 `.gitignore`로 가린 tracked 파일을 자동 감지. rewrite 전에 backup ref `refs/gk/forget-backup/<unix>/...` + `.git/gk/` 아래 평문 manifest 작성. `--dry-run`/`--yes` 지원. `git-filter-repo` 설치 필요 |
