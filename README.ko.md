...
- **한눈에 보는 상태.** `gk doctor`는 git 버전, pager, `$EDITOR`, 설정 유효성, 훅 상태, gitleaks 설치 여부, gk 백업 ref 누적에 대해 PASS/WARN/FAIL을 보고합니다. WARN/FAIL 줄마다 복사해 붙여넣을 수 있는 수정 명령이 같이 출력되며, `--ai` 또는 `--verbose`를 붙이면 옵셔널 AI 통합 항목도 함께 진단합니다.
- **설치 방식과 무관한 self-update.** `gk update`는 현재 실행 중인 바이너리 경로를 보고 Homebrew, `install.sh`, `go install` 중 어떤 경로로 깔렸는지 판단해 알맞게 분기합니다. brew면 `brew upgrade x-mesh/tap/gk`로 위임(바이너리가 Caskroom 아래라면 `--cask`까지 자동으로 붙입니다 — tap이 v0.55부터 formula→cask로 옮겨졌기 때문), manual이면 릴리즈에서 `gk_<os>_<arch>.tar.gz`를 받아 `checksums.txt`로 sha256을 검증한 뒤 같은 디렉토리에서 atomic rename(`/usr/local/bin`처럼 권한이 부족하면 sudo로 escalate, 이전 바이너리는 `.bak`으로 백업), go-install이면 `go install …@latest` 명령을 출력해 줍니다. `gk update --check`는 다운로드 없이 비교만 하고 새 버전이 있으면 exit 1을 반환해 cron/CI에서 게이트로 쓸 수 있습니다.
...
v0.55부터 tap이 formula에서 cask로 옮겨졌습니다. macOS/Linux 모두
`--cask` 플래그가 있어야 현재 릴리스를 잡습니다. v0.55 이전에 설치했고
`brew upgrade`가 계속 v0.54.0이라고 하면 `brew uninstall --formula
x-mesh/tap/gk`로 옛 formula를 한 번 제거한 뒤 위 cask install로 다시
까세요. `gk update`는 설치 경로를 보고 알아서 올바른 플래그를 붙입니다.
...
| `gk pull` | | fetch + upstream 통합. `--strategy rebase\|merge\|ff-only\|auto`; `@{u}` 우선 해석; HEAD가 이미 ancestor이면 `--ff-only`로 자동 전환. `--ai`는 충돌 시 AI로 자동 해결 후 통합 계속(`gk resolve --ai` + `gk continue` 구동) |
...
| `gk log` | `gk slog` | 짧은 컬러 커밋 로그. `--behind` / `--ahead`(옵션 `--fetch`)로 `@{u}` 기준 안 가져온/안 푸시한 커밋을 `gk pull` / `gk push` 전에 미리 본다. `--pulse`, `--calendar`, `--tags-rule`, `--impact`, `--cc`, `--safety`, `--hotspots`, `--trailers`, `--lanes` 시각화 |
| `gk chat` | | 저장소와 대화 — 모델이 읽기 전용 도구(git log/show/diff/blame/grep + 파일 읽기/목록 + 저장소 상태/snapshot/context 구조화 조회)를 스스로 호출해 근거를 찾아가며 답하는 에이전틱 루프. 모든 도구 호출이 한 줄씩 표시되고, 답변은 실제로 본 SHA와 `파일:라인`을 인용하며 작성되는 대로 스트리밍된다. REPL(`gk chat`, `/tokens`·`/compact`·탭완성 `/` 명령 지원) · one-shot(`gk chat "질문"`) · `--continue`/`--session <id>`로 세션 재개(`gk chat sessions`로 목록 확인, `sessions prune`으로 오래된 세션 정리). 샌드박스(repo 밖·symlink 탈출·.git·submodule 차단, deny_paths는 과거 커밋 출력에도 적용, 저장 전 redaction)와 상한(턴당 15라운드/192KB, 상한은 global config 전용)이 강제된다. tool-calling HTTP 프로바이더(anthropic/openai/groq/nvidia) 필요 |
| `gk local` | `gk lo` | 내 컴퓨터에만 있는 것들을 한 화면에: 작업트리 변경(미스테이지/스테이지/충돌), 안 푸시한 커밋(`@{u}` 우선, upstream 없으면 모든 원격 추적 ref로 폴백), 스태시. `-n N`, `--json` |
...
| `gk switch [name]` | `gk sw` | 브랜치 전환; `--fetch`로 remote branch를 먼저 갱신, `-m`/`--main`으로 main 이동, `-d`/`--develop`으로 develop. 다른 worktree에 체크아웃된 브랜치는 `WORKTREE` 칸에 표시; protected 브랜치를 linked worktree로 가져오려 하면 확인을 물음(`--detach`로 보기만) |
...
| `gk worktree add <name>` | | 상대 경로는 `<worktree.base>/<worktree.project>/<name>`(기본 `~/.gk/worktree/<repo>/<name>`)에 배치, 절대 경로는 그대로 사용. 고아 브랜치 충돌 시 인라인 재사용/삭제/취소 선택. 생성 후 worktree 초기화 여부를 물음(`--init`/`--no-init`로 프롬프트 생략) |
| `gk worktree init [path]` | | `.gk.yaml`의 `worktree.init`로 worktree의 gitignore된 상태 복원: `link`(`.env` 등 시크릿 symlink), `copy`(worktree별 파일), `run`(`npm ci`, `uv sync`). 멱등이라 셋업 재시도로도 사용. 설정이 없으면 패키지 매니페스트를 감지(루트, 또는 루트에 없으면 `frontend/`·`backend/` 같은 중첩 모노레포 프로젝트)해 블록을 제안(`--save`로 저장, `--dry-run`으로 미리보기) |
| `gk worktree list` | | `git worktree list --porcelain` 파싱 결과를 테이블 또는 `--json`으로 |
| `gk worktree remove <path>` | | 삭제; dirty는 force 재확인. locked worktree는 lock 소유 프로세스 생존 여부로 분기 — 죽은(stale) 잠금은 `--force`로 unlock+삭제, 살아있는 잠금은 거부하고 `--force-locked`로만 강제. stale admin 엔트리는 자동 prune |
| `gk prompt-info` | | 셸 프롬프트 통합용 짧은 라벨. `plain`(기본)은 linked 워크트리 안에서 `wt` (디렉터리명이 브랜치명과 다르면 `wt:<basename>`), 그 외에는 빈 출력. `--format=segment`는 git repo 안이면 `<repo>/<branch>`를 출력해 starship의 `$directory` + `$git_branch`를 단일 중복 제거 라벨로 대체. `--format=json`은 `{linked, repo, name, path, branch}`. 탐지는 `git rev-parse --git-dir` vs `--git-common-dir` 비교(~30ms). |
...
| `gk ignore <path>...` | | AI 없는 결정론 "이 파일을 git에 포함하지 않기": 각 경로를 `.gitignore`에 추가(디렉터리는 끝에 `/`)하고, 이미 tracked면 `git rm --cached`로 추적만 해제(작업 파일은 유지). `--commit`은 한 커밋으로 마무리, `--dry-run`은 미리보기. 이미 history에 들어간 경로를 지우려면 `gk forget` 사용. 자연어로도 호출됨: `gk do "<파일>을 git에 포함하고 싶지 않아"`가 이 명령으로 결정론 라우팅 |
...
| `gk next` | 현재 저장소 상태를 쉬운 언어로 설명하고 다음에 실행할 안전한 명령을 제안. AI provider가 없으면 로컬 규칙 기반 플랜으로 폴백. `--run`/`-r`은 1순위 추천 명령(자유형 AI 출력이 아니라 gk의 결정적 allowlist)을 확인 후 실행하며, 위험 명령은 거부 |
...
| 명령어 | 설명 |
|---|---|
| `gk doctor` | 환경 상태 보고 (git/pager/editor/config/hooks/gitleaks/backup-refs); `--ai` 또는 `--verbose`로 AI 프로바이더 행 추가; `--json` for CI |
| `gk update [--check] [--force] [--to <vX.Y.Z>]` | self-update. `os.Executable()` 결과로 brew / `install.sh` / `go install`을 구분해 분기합니다. brew → `brew upgrade x-mesh/tap/gk` 위임(바이너리가 Caskroom 아래라면 `--cask` 자동 추가 — v0.55부터 tap이 formula→cask로 이전됨); manual → 릴리즈에서 `gk_<os>_<arch>.tar.gz` 받아 `checksums.txt`로 검증 후 atomic rename(필요 시 sudo escalate, 이전 바이너리는 `.bak` 백업); go-install → `go install …@latest` 명령 출력. `--check`는 다운로드 없이 비교만 (새 버전 있으면 exit 1), `--to`는 manual 설치에서 특정 태그 핀 |
| `gk init [--only <target>] [--kiro] [--ai-gitignore] [--force]` | 프로젝트 분석 후 `.gitignore`와 `.gk.yaml`을 한 번에 스캐폴드. `--only gitignore\|config\|ai`로 범위 제한; `--kiro`는 `.kiro/steering/` 작성; `--ai-gitignore`는 확인 후 설정된 AI provider에 추가 ignore 패턴을 요청. 인터랙티브 huh 폼이 로컬 플랜을 미리 보여줌 |
...
`gk doctor --ai` 또는 `gk doctor --verbose` 출력에서 각 provider 설치/인증 상태를 확인할 수 있습니다.
...
make install        # ~/.local/bin/gk-dev에 설치 (안전 기본값, Homebrew와 충돌 없음)
make install-gk     # ~/.local/bin/gk에 설치 (dev 빌드를 gk로 사용)
