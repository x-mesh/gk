...
| `gk find <query>` | `gk search` | One-call history search: commit **messages** (`--grep`), changed **content** (the `-S` pickaxe) and **paths** are searched at once, across every ref, and each result says which matched. The turn cost of archaeology is not one query — it is not knowing which query will hit, so raw git pays a turn per guess (`--grep` miss → `-S` miss → path hit). Multi-mode matches rank first, then recency. `--path`/`--since`/`--author`/`--ref` narrow; `--no-content` drops the pickaxe (the slow mode on big repos); `--json` for agents. Does **not** answer `git log A..B` (a range comparison, not a search) — use `gk log --ahead/--behind --base` |
