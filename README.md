...
gk context --delta           # repeat orientation: only the fields that changed since the last call
...
gk agents install            # install globally in ~/.claude/CLAUDE.md + ~/.codex/AGENTS.md
gk agents install --local    # opt into repo-root CLAUDE.md / AGENTS.md
...
|---|---|---|
| `gk clone [owner/repo \| alias:owner/repo \| url]` | | Clone with short-form URL expansion. Bare `owner/repo` expands to `git@github.com:owner/repo.git` (ssh default, configurable). `--ssh`/`--https` override. `clone.hosts` maps aliases (`gl:`, `work:`); an alias with an `owner` becomes an account profile — `alias:repo` completes the owner, and optional `ssh_host` swaps an `~/.ssh/config` Host alias into ssh URLs for multi-account key separation. With **no argument**, browse and pick from the github.com account profiles' repositories (listed via `api.github.com` directly — no `gh` binary; token from `GH_TOKEN`/`GITHUB_TOKEN`/`gh` config, else unauthenticated/public-only). Optional `clone.root` + `clone.post_actions: [hooks-install, doctor]`. |
...
| `gk resolve` | | Resolve the current conflicts AND finish the operation (drives `git <op> --continue`). `--safe` runs the deterministic tier only — identical sides, whitespace-only, one-side-unchanged-from-base, union files (`CHANGELOG.md`, `go.sum`) — no AI, nothing guessed, the rest stay conflicted; `--ai` opts into per-hunk semantic resolution (the mechanical tier runs first and shrinks the AI surface); `--strategy ours\|theirs` stays a pure side-take. Batch runs stage nothing until a verification gate passes (conflict-marker scan + `resolve.verify` commands) and restore the conflicted state on failure; `resolve.rerere` (default on) reuses recorded resolutions first. `resolve.min_confidence` holds back AI hunks the model itself is unsure about — they stay conflicted and the withheld answers ship as `proposals[]` in the paused report |
...
| `gk status` | `gk st` | Concise working-tree status (staged / unstaged / untracked / conflicted + ahead/behind), submodule-aware with `next:` hints. Pass `--ai` for a plain-language explanation, `-f`/`--fetch` to refresh ↑N ↓N, `--watch` for a live change-feed (fsnotify timeline of which files — and which **functions**, via git's hunk contexts — change as you/an agent edit, with the latest commit's short-sha, age, and subject pinned in the header so you can tell a fresh commit from an old one; `[s]` toggles the full status dashboard, ↑/↓ and `pgup`/`pgdn` scroll back through the feed's 1000-event history), or `--exit-code` for scripts. Opt-in `--vis gauge,bar,progress,types,staleness,tree,conflict,churn,risk` overlays |
| `gk find <query>` | `gk search` | One-call history search: commit **messages** (`--grep`), changed **content** (the `-S` pickaxe) and **paths** are searched at once, across every ref, and each result says which matched. The turn cost of archaeology is not one query — it is not knowing which query will hit, so raw git pays a turn per guess (`--grep` miss → `-S` miss → path hit). Multi-mode matches rank first, then recency. `--path` scopes a query, while `--path` without a query shows that file's rename-following history; `--since`/`--author`/`--ref` narrow further; `--no-content` drops the pickaxe (the slow mode on big repos); `--json` for agents. Does **not** answer `git log A..B` (a range comparison, not a search) — use `gk log --ahead/--behind --base` |
| `gk log` | `gk slog` | Short colored commit log; `--since 1w`, `--graph`, `--limit N`. `--behind` / `--ahead` (with `--fetch` to refresh, `--base` to compare against the base branch instead of `@{u}`) preview incoming/outgoing commits before `gk pull` / `gk push`. A `──┤ ↑ N unmerged → <base> ├──` divider marks where the branch runs ahead of its base, drawn by default whenever you're off the base branch (and merged with the `--safety` push divider when they land on the same row). Opt-in `--pulse`, `--calendar`, `--tags-rule`, `--impact`, `--cc`, `--safety`, `--merged`, `--hotspots`, `--trailers`, `--lanes` visualizations. `--ai` appends a plain-language summary of the shown range below the list — a reading companion (not a changelog generator), grounded in the same hotspot/WIP/breaking/CC signals the viz flags compute, respecting `output.lang`/`--lang`; `--json --ai` wraps the entry array as `{entries, ai_summary}` (plain `--json` is unchanged) |
| `gk chat` | | Talk to your repository — an agentic loop where the model calls read-only tools itself (git log/show/diff/blame/grep + file read/list + structured repo status/snapshot/context queries + `gk_suggest`, which resolves a gk command against this build's own command tree so a suggestion cannot name a command that does not exist; every call shown live with summarized args and result) to investigate before answering with cited SHAs and `file:line` evidence, streaming the answer as it's written. REPL (`gk chat`, with `/tokens`/`/compact` and Tab-completed `/` commands), one-shot (`gk chat "질문"`), `--continue`/`--session <id>` resumes a session (`gk chat sessions` lists them, `sessions prune` expires old ones). Sandboxed (repo-root containment incl. symlink resolution, deny_paths enforced even on historic `git show` output, redact-before-persist) and capped (15 rounds / 192KB per turn, limits honored from the global config only). Needs a tool-calling HTTP provider (anthropic/openai/groq/nvidia) |
| `gk local` | `gk lo` | One screen for everything that lives only on this machine: working-tree changes (unstaged/staged/conflicts), unpushed commits (uses `@{u}`, falls back to any remote-tracking ref when there's no upstream), and stash. `--all` widens the scan to every local branch and worktree — the current branch alone cannot answer "is any work stranded on this machine" — adding a `branches[]` breakdown and making `clean` the repo-wide rollup (`--json` reports `scope` so a caller knows what `clean` covers; anything unverifiable is `unknown` and forces `clean: false`). `-n N`, `--json` |
...
| `gk watch` | `gk w` | Live supervision at the right altitude: several worktrees, multi-repo flags, **or run from a non-repo parent directory (auto-scans one level down — `cd ~/work && gk watch`)** → the **dashboard** — every worktree at once (branch, ↑↓, dirty/conflict, last-changed file, paused op), a merged **change feed** in `gk status --watch` form (file · **function** · ±; `--feed-stats=false` for names only), `enter` panel cycle, `w` in-place zoom into a worktree's feed (`esc` back, `[`/`]` hop), `pgup`/`pgdn` scroll the feed back through its history (`end` returns to the live tail; arrivals never move the lines you are reading), `f`/`s` filter·sort (multi-repo defaults to the **active** filter — only worktrees being worked on, header shows `5/21`; `--filter all` for everything), `space` repo fold (clean repos start folded), activity age turns green while a worktree is live. Exactly one worktree → straight into the `gk status --watch` feed. `--json`/`GK_AGENT` one-shot snapshot and `--events` NDJSON stream (file-changed / status-changed / op-start / op-end / land-ready + `fleet.notify` hooks) always use the dashboard contract. `gk fleet` is the deprecated former name (one release, hidden alias); config keys stay `fleet.*` |
...
| `gk precheck <target>` | Dry-run merge conflict scan via `git merge-tree` — names the conflicting **function**, not just the file (`✗ app.py · in process`); exit 3 on conflicts; `--json` for CI (`conflicts[]` + append-only `details[]`) |
...
| `gk undo` | | Reflog-based HEAD restore; pick a past HEAD (or `--to <ref>`) and reset, leaving a backup ref at `refs/gk/undo-backup/...`. `--soft` uncommits (HEAD only — staged stays staged; bare non-interactive `--soft` defaults to `HEAD~1`), `--hard` discards the working tree; JSON result `{schema, result, from, to, backup_ref, mode}` |
| `gk unstage [path...]` | | Drop files from the staging area without touching their contents (the safe `git reset [-q] HEAD -- <paths>` form; no paths = everything staged). History-moving resets are out of scope — that's `gk undo` |
| `gk apply <patch-file>...` | | Apply patch files with an automatic strategy ladder (plain → `--recount` → `--unidiff-zero` → `--3way`), recording which rung applied each patch. Multi-patch is all-or-nothing (rolls back on any failure); `--staged`/`--cached` applies index-only, `--check` probes without applying (and predicts 3-way conflicts), `--reverse` undoes an applied patch. JSON result `{applied:[{patch,strategy}], failed, rolled_back}` with a `result` marker (`applied`/`check`/`dry-run`) |
| `gk reset` | | Fetch a target, then hard-reset the current branch to it — gated on a confirm (`-y` skips). Defaults to the configured upstream; `--to <ref>` (or a bare positional, `gk reset main`) resets to any ref, so it replaces `git reset --hard <ref>`. `--to-remote` uses `<remote>/<current>`, `--clean` also removes untracked files, `--dry-run` previews. **Destructive** — it writes no backup ref; the pre-reset HEAD is only in the reflog (`gk undo`). For a reset that *does* back up first, use `gk timemachine restore` |
...
| `gk discard <path>...` | | Discard uncommitted changes to paths — after auto-saving a `refs/wip` snapshot of the whole tree (untracked included), so the discarded work stays recoverable via `gk snapshot restore`. The discard itself is what `git checkout -- <path>` does: worktree restored from the INDEX; staged changes stay staged, untracked files are never touched (reported instead), unmerged paths block toward `gk resolve`. No confirmation prompt — the snapshot is the gate, and a failed snapshot aborts the discard. `--dry-run` previews; JSON result `{result, discarded, untracked_kept, snapshot_ref, snapshot_sha}` |
...
| `gk pr create` | Open a pull request from the current branch. Base from `--base` > `base_branch` > the remote's default; title/body from the commits, `--ai` drafts the body, `--title`/`--body`/`--body-file` override. `--draft`, `--web`, `--dry-run`. Refuses when the branch and its remote counterpart differ, with a `gk push`/`gk pull` remedy — it never pushes for you |
...
| `gk drivers [install\|uninstall\|status]` | Map file extensions to git's **built-in language diff drivers** in `.git/info/attributes` (repo-local, never version-controlled, shared by all linked worktrees — teammates unaffected). Sharpens every hunk function context gk reads: live-feed symbols (`st --watch`, `fleet --feed-stats`), `diff --digest` symbols, and conflict symbols — e.g. a CSS hunk names its selector, an indented Python method resolves correctly. Marker-fenced block, idempotent, `uninstall` round-trips |
...
| `gk agents [print\|install\|check]` | Keep gk's usage contract — the block that tells an agent to route git through gk instead of probing with raw git — current inside instruction files. `install` defaults to the shared `~/.claude/CLAUDE.md` and `~/.codex/AGENTS.md` (parent dirs created as needed); `--global` remains an explicit equivalent and `--local` targets repo-root `CLAUDE.md`/`AGENTS.md`; `--full` opts into the longer reference block. `check` reports each file's block status and version across both scopes — local and global — and flags any left on an older gk; `print` dumps the compact block to stdout (`--full` for the detailed block). `--tuned` (on `print`/`install`) adds one data-backed line naming this environment's top raw-git turn leak from `~/.gk/audit-history.jsonl`. The contract ships inside the binary, so it always matches the installed gk |
| `gk agents hook [install\|uninstall\|status]` | The **enforcement** companion to the contract block: register a Claude Code PreToolUse(Bash) hook in `settings.json` that steers raw git to git-kit at the point of a command (`--mode collapse` is the default and denies only a repeated same-group probe; `--mode warn` only surfaces a note; `--mode block` denies every covered raw git). `install` also wires a **UserPromptSubmit prefetch** hook (`--no-prompt` opts out): a prompt that clearly asks for a git action gets a one-line orientation (branch, ↑↓, dirty, paused op + resume) injected before the model thinks — the first probe turn disappears; conservative gate, fail-open, ≤800 chars, ~29ms. `--stop-commit` adds an opt-in **Stop checkpoint** hook: a session ending with uncommitted work gets one `gk commit --wip` checkpoint (fail-open, loop-guarded, skips in-progress Git operations including linked worktrees, 120s ceiling) that a later `gk commit` folds into real commits — `--stop-only` registers just that one and leaves the other events untouched (Claude merges PreToolUse across scopes, so don't install it twice). Surgical, non-destructive settings edit (only the gk entries, `.bak` first, `--dry-run` to preview); `--global` for `~/.claude/settings.json`. `uninstall` reverts. Backed by `gk hint` |
| `gk hint [command]` | Map one raw `git` command (argument or stdin) to the git-kit verb that covers it — the mapping behind `gk agents hook` and `gk session audit`. `--json` emits `{covered, kind, covered_by, suggestion, matched, caution?}`; `--exit-code` exits 1 when a replacement exists. Plumbing (`rev-parse`, `config`, …) and already-git-kit commands report not-covered. Index-source pathspec discards (`git checkout -- <path>`, `git restore <path>`) map to `gk discard`; the ref-source forms (`checkout <ref> -- <path>`, `restore --source`, `--ours`/`--theirs`) stay not-covered but carry a `caution`: run `gk snapshot` first so the discard is recoverable |
| `gk session audit [paths...]` | Read local Codex/Claude JSONL sessions (local-only, nothing leaves the machine) and report where agents still fall back to raw git: adoption rate, covered/uncovered findings with per-subcommand breakdown (`one_shot` marks low-turn-leverage gaps), and a ready-to-run `batch --plan` payload per shell chain. `--since 30d` windows the corpus by file mtime; `--metric=turns\|both` adds the turn-reduction view (`--viz` graph) plus `turns.gk_reprobe` — repeated gk reads one call would have answered, reported beside (never summed into) the adoption number; `--record` appends to `~/.gk/audit-history.jsonl` (now with `gk_turns`/`gk_reprobe_saved`) and `--trend` reads it back (in `--json`, as `result.trend[]`). The JSON output is token-lean by default (`files[]` omitted, `runs[].commands`/`findings[].evidence` capped) — `--files` restores the per-file breakdown and `--full` collects and emits uncapped evidence, `--summary` emits the decision-grade subset; `--max-files N` caps the newest files scanned |
| `gk session digest [transcript]` | Compress one agent session's git activity (repos, branches, commit subjects, integration attempts + errors, unfinished-work signal, re-probed groups) into a ~1 KB resume/handoff block. `--last[=N]` picks the N-th-newest local session (`--last=2` for the previous one when run from inside a live session); local, read-only, standard envelope under `--json`/`GK_AGENT` |
...
gk commit [flags]
  ...
  -n, --no-verify                  bypass the noise + secret guards and the privacy-gate abort threshold (secrets are reported, then committed; payload redaction to remote AI still applies)
      ...
      --wip                        write one WIP(scope) checkpoint commit instead of a semantic history; one provider call, falls back to a plain checkpoint message when the AI is unavailable (implies --force and --no-wip-unwrap)
...
- **Privacy gate.** For remote providers (`Locality=remote`), the outbound payload is scrubbed: secrets, paths matching `deny_paths`, and sensitive patterns are replaced with tokens like `[SECRET_1]` or `[PATH_1]`. The run aborts if more than ten **distinct** secrets show up in a single payload. A value repeated across the diff counts once (one secret in N places, not N secrets), and documented example keys (`AKIAIOSFODNN7EXAMPLE`) or obvious placeholders are **redacted but never counted** — the same judgement `gk push` uses — so test fixtures cannot spend the budget that exists to catch a real credential. The report marks each exempted finding `(not counted: placeholder | duplicate)`. Raise `ai.commit.privacy.max_secrets`, or skip that threshold with `--skip-privacy` (`-n`/`--no-verify` implies it; redaction is unaffected either way). `--show-prompt` lets you inspect the redacted version. With `ai.commit.audit` on, the redactions are logged to `.gk/ai-audit.jsonl`.
...
- **Backup ref.** Each run writes `refs/gk/ai-commit-backup/<branch>/<unix>` before committing. `gk commit --abort` hard-resets HEAD to it — and, since that discards the working tree too, first writes a second safety-net ref at the pre-abort HEAD (`refs/gk/ai-commit-abort-backup/<branch>/<unix>`) so an abort after a fully successful commit never loses that commit's content; the abort output prints where it went.
