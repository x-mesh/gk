...
| `gk agents hook [install\|uninstall\|status]` | The **enforcement** companion to the contract block: register a Claude Code PreToolUse(Bash) hook in `settings.json` that steers raw git to git-kit at the point of a command (`--mode warn` default surfaces a note; `--mode collapse` denies only a repeated same-group probe; `--mode block` denies every covered raw git). `install` also wires a **UserPromptSubmit prefetch** hook (`--no-prompt` opts out): a prompt that clearly asks for a git action gets a one-line orientation (branch, ↑↓, dirty, paused op + resume) injected before the model thinks — the first probe turn disappears; conservative gate, fail-open, ≤800 chars, ~29ms. Surgical, non-destructive settings edit (only the gk entries, `.bak` first, `--dry-run` to preview); `--global` for `~/.claude/settings.json`. `uninstall` reverts. Backed by `gk hint` |
