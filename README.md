...
| `gk worktree run <branch> -- <cmd>` | | Create (or reuse) a worktree for `<branch>`, run `<cmd>` with the worktree as its cwd, and exit with the command's code — the single-shot CLI form of an isolated parallel task. `--cleanup` reclaims the worktree when the command succeeds (and deletes the branch if this call created it); `--from <ref>` bases a new branch, `--init`/`--no-init` force or skip the gitignored-state bootstrap. |
| `gk fleet` | | Live multi-worktree supervision dashboard: every worktree at once (branch, ahead/behind, dirty/conflict, current) for watching parallel work — e.g. several AI agents each in a worktree. The TUI polls on `--interval` (default 2s; `j`/`k` move, `r` refresh, `q` quit) and rolls each up to a `status` (clean/dirty/conflict/ahead/behind/diverged). `--json` (or `GK_AGENT=1`) emits a one-shot `{path,branch,current,ahead,behind,dirty,status}` snapshot — the contract an agent polls, TUI as consumer. Reuses `gk worktree list`'s enrichment. |
