...
| `gk chat` | | Talk to your repository — an agentic loop where the model calls read-only tools itself (git log/show/diff/blame/grep + file read/list, every call shown live) to investigate before answering with cited SHAs and `file:line` evidence. REPL (`gk chat`), one-shot (`gk chat "질문"`), `--continue` resumes the session stored under `.git/gk-chat/`. Sandboxed (repo-root containment incl. symlink resolution, deny_paths enforced even on historic `git show` output, redact-before-persist) and capped (15 rounds / 192KB per turn, limits honored from the global config only). Needs a tool-calling HTTP provider (anthropic/openai/groq/nvidia) |
