...
| `gk session audit [paths...]` | Read local Codex/Claude JSONL sessions (local-only, nothing leaves the machine) and report where agents still fall back to raw git: adoption rate, covered/uncovered findings with per-subcommand breakdown (`one_shot` marks low-turn-leverage gaps), and a ready-to-run `batch --plan` payload per shell chain. `--since 30d` windows the corpus by file mtime; `--metric=turns\|both` adds the turn-reduction view (`--viz` graph); `--record` appends to `~/.gk/audit-history.jsonl` and `--trend` reads it back (in `--json`, as `result.trend[]`). The JSON output is token-lean by default (`files[]` omitted, `runs[].commands`/`findings[].evidence` capped) — `--files`/`--full` restore detail, `--summary` emits the decision-grade subset; `--max-files N` caps the newest files scanned |
