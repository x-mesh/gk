...
| `gk agents hook install [--mode block\|collapse\|warn] [--global] [--dry-run]` | Register a Claude Code PreToolUse(Bash) hook in `settings.json` that steers raw git to git-kit at the moment a command runs (default `.claude/settings.json`; `--global` for `~/.claude/settings.json`) |
