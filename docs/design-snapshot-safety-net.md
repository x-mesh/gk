...
## 4. Phase 2 — 트리거 자동화 (구현됨, 4.2 제외)

Phase 1은 명령만 제공한다. 진짜 "자동" 안전망이 되려면 트리거가 필요하다.
**2026-07-07 구현 반영:** 4.1은 `gk snapshot hook install|status|uninstall`로,
4.3의 보존 정책은 `gk snapshot prune` + `snapshot.retention_days`로, diff 비교는
`gk snapshot diff [n]`으로 구현됐다([phase2-improvements](phase2-improvements.md)).
4.2 watch 데몬만 미적용으로 남는다.

### 4.1 Claude Code Stop hook (권장, 가장 단순) — 구현됨: `gk snapshot hook install`
...
- 구현: `gk snapshot hook install`이 위 JSON을 append-only로 병합한다 —
  기존 Stop 항목·다른 설정 키 보존, 멱등, 파싱 불가 파일은 거부(절대
  재작성하지 않음). 기본 대상은 `~/.claude/settings.json`, `--project`는
  repo의 `.claude/settings.json`, `--settings <path>`는 임의 경로.

### 4.2 시간 기반 watch 데몬 (미적용)
...
- 보존: `gk snapshot prune [--keep-days N] [--all]` + `snapshot.retention_days`
  (기본 0 = off; > 0이면 저장 때마다 조용히 best-effort expire, 전부 만료된
  ref는 삭제해 유령 ref를 남기지 않는다).
- 비교: `gk snapshot diff [n] [--stat]` — restore가 적용할 방향 그대로
  스냅샷 ↔ 현재 작업트리를 보여준다. (`gk restore --diff` 대신 snapshot
  서브커맨드로 배치 — restore 네임스페이스 충돌 회피, §3과 동일한 이유)
