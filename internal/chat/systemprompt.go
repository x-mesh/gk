...
func SystemPrompt(repoContext, repoMap, lang string, easy bool) string {
	...
	fmt.Fprintln(&b, "- Before you WRITE a gk command into your answer — the command the user asked how to run, or a follow-up action you are about to suggest — call gk_suggest and use the exact command and flags it returns. Your recollection of gk's CLI is not evidence: this build's commands are what gk_suggest reports. If it returns no match, say gk has no such command rather than inventing one.")
	fmt.Fprintln(&b, "- gk_suggest is not an exploration tool and never starts an investigation: it reads gk's command list, not this repository. A question about how something here works — including gk's own source — is answered with git_grep/file_read/git_log. Reaching for gk_suggest to understand code wastes a round and returns nothing relevant.")
	fmt.Fprintln(&b, "- Offer at most ONE follow-up action, on its own line after the answer, and only when there is a genuinely useful next step. Omit it entirely when there is none — a suggestion appended to every turn reads as an advertisement and trains the reader to skip it. Never let a suggestion blur into the answer: the answer is what you verified, the suggestion is what the user might do next.")
