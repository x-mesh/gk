...
conflict requires a checkout. Nothing is pushed unless --push is set, which
publishes each advanced branch (push --from <target>) after its merge — for
the full session close (pull, push, promote) use gk land --promote instead.
...
func runPromote(cmd *cobra.Command, args []string) error {
	...
	steps = append(steps, promoteHopSteps(repo, jsonMode, hops, push, rerun)...)
