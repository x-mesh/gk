...
// Package lineedit is a vendored copy of golang.org/x/term's Terminal line
// editor (v0.42.0), patched to be East-Asian-wide-rune aware. Upstream assumes
// every rune occupies exactly one terminal cell, so its cursor math corrupted
// lines containing 한글/CJK/wide-emoji (residue on delete, misplaced cursor).
// Unlike bubbletea's inline renderer, this editor leaves the HARDWARE cursor at
// the logical caret position, so terminal IME preedit composition renders in
// the right place — the reason we keep an x/term-style editor for the REPL.
//
// Changes from upstream are marked "wide-rune:" and live in visualLength,
// moveCursorToPos, setLine, and eraseNPreviousChars. Everything else is
// byte-for-byte upstream; keep it that way to ease future resyncs.
package lineedit
...
func (t *Terminal) moveCursorToPos(pos int) {
	...
	// wide-rune: pos is a rune index into t.line, but the screen column is the
	// cell width of everything before it, not the rune count. Callers keep
	// t.line consistent with pos before calling (see setLine).
...
func (t *Terminal) setLine(newLine []rune, newPos int) {
	if t.echo {
		// wide-rune: pad the tail by the CELL difference (not the rune count)
		// so a shorter new line fully overwrites a wider old one, and assign
		// t.line BEFORE the final moveCursorToPos so it measures the new line.
		oldCells := visualLength(t.line)
		t.moveCursorToPos(0)
		t.writeLine(newLine)
		for i := visualLength(newLine); i < oldCells; i++ {
			t.writeLine(space)
		}
		t.line = newLine
...
func (t *Terminal) eraseNPreviousChars(n int) {
	...
	// wide-rune: the deleted runes vacate their CELL width on screen, which is
	// what we must blank out and advance over — not the rune count n. Measure
	// before the copy below overwrites them.
	vacatedCells := visualLength(t.line[t.pos-n : t.pos])
	t.pos -= n
	...
	if t.echo {
		...
		for i := 0; i < vacatedCells; i++ {
			t.queue(space)
		}
		t.advanceCursor(vacatedCells)
		t.moveCursorToPos(t.pos)
	}
}
...
// visualLength returns the number of terminal cells the visible glyphs in s
// occupy. wide-rune: upstream counted one cell per rune; we sum runewidth so
// East-Asian wide runes (한글/CJK/wide-emoji) count as two. Because writeLine,
// moveCursorToPos, and advanceCursor all route their column math through this,
// making it cell-accurate fixes cursor placement and erase across the editor.
