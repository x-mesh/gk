...
func TestHint(t *testing.T) {
	cases := []struct {
		...
		// The index-source per-path discard is covered by gk discard.
		{"checkout restore", "git checkout -- app.go", true, "raw-discard"},
		// Not covered: plumbing, stash subcommands git-kit stash lacks,
		// already git-kit, non-git.
...
// that destroys work. Since gk discard, the caution covers only the forms that
// verb does NOT replace: discards whose source is a ref or a conflict side,
// not the index.
func TestHint_DestructiveDiscardCautionsWithoutClaimingCoverage(t *testing.T) {
	for _, cmd := range []string{
		...
		// The whole point: warn WITHOUT inventing a replacement — gk discard
		// restores from the index, and these forms do not.
		if res.Covered {
			t.Errorf("Hint(%q).Covered = true — gk has no verb for this form", cmd)
		}
	}
}

// The index-source discards graduate from the caution to a real answer:
// gk discard is the same operation with the snapshot built in, so the hint
// names the verb and no longer warns. covered_by[0] matters — the hook tells
// the agent to run exactly that entry.
func TestHint_IndexSourceDiscardNamesGkDiscard(t *testing.T) {
	for _, cmd := range []string{
		"git checkout -- src/a.rs",
		"git restore src/a.rs",
		"git restore --worktree src/a.rs",
	} {
		res := Hint(cmd)
		if !res.Covered || res.Kind != "raw-discard" {
			t.Fatalf("Hint(%q) = %+v, want covered raw-discard", cmd, res)
		}
		if len(res.CoveredBy) == 0 || res.CoveredBy[0] != "git-kit discard <paths>" {
			t.Errorf("Hint(%q).CoveredBy = %v, want git-kit discard <paths> first", cmd, res.CoveredBy)
		}
		if res.Caution != "" {
			t.Errorf("Hint(%q) cautions despite coverage — the replacement ships the snapshot built in", cmd)
		}
	}
}
...
// The destructive segment is a ref-source restore — the form gk discard does
// not cover, so its caution must ride alongside the other segment's coverage.
func TestHint_CautionSurvivesAlongsideCoverage(t *testing.T) {
	res := Hint("git restore --source=HEAD~1 a.rs && git status")
	if !res.Covered {
		t.Fatalf("the git status segment is covered, got %+v", res)
	}
	if res.Caution == "" {
		t.Error("the ref-source discard segment must still raise its caution")
	}
}
